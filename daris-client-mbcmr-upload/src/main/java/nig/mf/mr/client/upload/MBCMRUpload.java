...
public class MBCMRUpload {
	...
	// The authenticating security token must be made with :app == to this
	// string.
	// The token must also hold the appropriate permissions to allow it to
	// access the
	...
	private static class Options {
		...
		// This is the actual Study CID (it's not an input option, it's managed
		// in this application)
	...
	public static void main(String[] args) throws Throwable {
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equalsIgnoreCase(HELP_ARG)) {
				...
				System.err.println("MBCMRUpload: error: unexpected argument = "
						+ args[i]);
				printHelp();
				System.exit(1);
			}
		...
		//
		if (ops.path == null)
			ops.path = DEFAULT_SRC_PATH;
		MBCRawUploadUtil.checkPath(ops.path);

		if (ops.id == null) {
			System.err
					.println("MBCMRUpload: you must specify the -id argument as usage is now only with the DaRIS data model");
		}

		// Create logger
		PrintWriter logger = MBCRawUploadUtil.createFileLogger(ops.logger,
				ops.logpath, DEFAULT_LOGGER_PATH);

		if (!ops.chksum) {
			if (ops.delete) {
				MBCRawUploadUtil
						.log(logger,
								"*** Disabling deletion of input as check sum has been turned off");
				ops.delete = false;
			}
		}
		//
	...
	private static void upload(PrintWriter logger, Options ops)
			throws Throwable {

		// Make connection to MF server
		Connection cxn = ClientConnection.createServerConnection();
		UserCredential cred = ClientConnection.connect(cxn, TOKEN_APP,
				ops.decrypt);

		// Check CID is for this server
		nig.mf.pssd.client.util.CiteableIdUtil.checkCIDIsForThisServer(cxn,
				ops.id, true);
		...
		if (depth == CiteableIdUtil.STUDY_ID_DEPTH) {
			...
			if (rawMeta == null) {
				throw new Exception(
						"The supplied destination Study does not appear to be a Siemens Raw MR Study (missing document)");
			}
		}
		...
		if (path.isDirectory()) {
			...
			List<File> files = sortFiles(FileUtils.listFiles(path, fileFilter,
					null));
			if (files != null && files.size() > 0) {
				...
				MBCRawUploadUtil.log(logger, "*** No files to upload in : "
						+ path.toString());
			}
		} else {
	...
	private static List<File> sortFiles(Collection<File> files)
		...
		Collections.sort(list, new Comparator<File>() {
			...
			public int compare(File f1, File f2) {
				...
				if (n1.startsWith(FILE_NAME_PREFIX)
						&& !n2.startsWith(FILE_NAME_PREFIX)) {
					return 1;
				}
				if (!n1.startsWith(FILE_NAME_PREFIX)
						&& n2.startsWith(FILE_NAME_PREFIX)) {
				...
				int i1 = n1.indexOf("_FID");
				int i2 = n2.indexOf("_FID");
				if (i1 != -1 && i2 == -1) {
					return 1;
				}
				if (i1 == -1 && i2 != -1) {
				...
				int fid1 = Integer.parseInt(n1.substring(i1 + 4));
				int fid2 = Integer.parseInt(n2.substring(i2 + 4));
				if (fid1 > fid2) {
					return 1;
				}
				if (fid1 < fid2) {
				...
				/*
				 * FID are the same. now compare MID
				 */
				int j1 = n1.indexOf("_MID");
				int j2 = n2.indexOf("_MID");
				int k1 = n1.indexOf("_", j1 + 4);
				int k2 = n2.indexOf("_", j2 + 4);
				if ((j1 != -1 && k1 != -1) && (j2 == -1 || k2 == -1)) {
					return 1;
				}
				if ((j1 == -1 || k1 == -1) && (j2 != -1 && k2 != -1)) {
					return -1;
				}
				if ((j1 == -1 || k2 == -1) && (j2 == -1 || k2 == -1)) {
					return 0;
				}
				int mid1 = Integer.parseInt(n1.substring(j1 + 4, k1));
				int mid2 = Integer.parseInt(n2.substring(j2 + 4, k2));
				if (mid1 > mid2) {
					return 1;
				}
				if (mid1 < mid2) {
	...
	private static void uploadFile(ServerClient.Connection cxn, File file,
			Options ops, UserCredential cred, PrintWriter logger)
			throws Throwable {
		...
		try {
			...
			MBCRawUploadUtil
					.log(logger,
							"   *** Failed to parse file meta-data header - skipping file");
			return;
		}
		pm.print();
		...
		if (ops.id == null) {
			...
			String assetID = MBCRawUploadUtil.findPatientAssetFromDICOM(cxn,
					method, pm.getFirstName(), pm.getLastName(), pm.getID(),
					null, MF_NAMESPACE, logger);
			...
			if (depth < CiteableIdUtil.STUDY_ID_DEPTH) {
				...
				// multiples
				subjectID = MBCRawUploadUtil.findSubjectAsset(cxn,
						MF_NAMESPACE, ops.id, false, method, pm.getFirstName(),
						pm.getLastName(), pm.getID(), logger);

				// Upload, creating Study and DataSets
				if (subjectID != null) {
					MBCRawUploadUtil.log(logger, "     Subject asset = "
							+ subjectID);
					createPSSDAssets(cxn, file, pm, subjectID, cred, ops,
							logger);
				} else {
					// Skip uploading this one. A message will have been issued
					// as to why
				}
			} else {

				// Handle the case where a Study is specified. We have already
				// checked that it's a valid Raw Siemens Study
				...
				if (rawSeriesCID != null) {
					MBCRawUploadUtil.log(logger,
							"     *** Found existing raw Series (by name) with ID = '"
									+ rawSeriesCID + "' - skipping");
					return;
				}
				...
				if (rawSeriesCID == null) {
					...
					MBCRawUploadUtil.log(logger,
							"     *** Found existing raw DataSet ID (by name) = "
									+ rawSeriesCID + " - skipping");
					chkSumOK = true; // This will trigger a deletion since the
										// file has already been successfully
										// uploaded
				}

				// Clean up input file if upload successful
				if (ops.delete && chkSumOK)
					deleteFile(file, logger);
			}
		}
	}

	private static String createDICOMAssets(ServerClient.Connection cxn,
			File file, MRMetaData pm, String patientAssetID,
			UserCredential cred, Options ops, PrintWriter logger)
			throws Throwable {

		// Look for MR raw STudy associated with this Patient
		String rawStudyID = null;// findRawStudy (cxn, pm, patientAssetID,
									// null);

		// Create Study if needed
		if (rawStudyID == null) {
			rawStudyID = createRawStudy(cxn, pm, patientAssetID, null, cred);
			MBCRawUploadUtil.log(logger, "     Created raw Study asset = "
					+ rawStudyID);
		} else {
		...
		if (rawSeriesID == null) {
			...
			MBCRawUploadUtil.log(
					logger,
					"      File size = "
							+ FileUtils.byteCountToDisplaySize(tsize));
			rawSeriesID = createRawSeries(cxn, file, pm, rawStudyID, null,
					ops.expire);
			if (rawSeriesID == null) {
				throw new Exception("Failed to create series asset");
			}
			MBCRawUploadUtil.log(logger, "     Created raw Series asset = "
					+ rawSeriesID);
			if (ops.chksum) {
				...
				if (chkSumDisk.equalsIgnoreCase(chkSumAsset)) {
					...
					MBCRawUploadUtil.log(logger,
							"       Checksums do not match. Checksums are:");
					MBCRawUploadUtil.log(logger,
							"           Input file      = " + chkSumDisk);
					MBCRawUploadUtil.log(logger,
							"           Mediaflux asset = " + chkSumAsset);
					//
					AssetUtil.destroy(cxn, rawSeriesID, null);
					MBCRawUploadUtil.log(logger,
							"           Destroyed Mediaflux asset");
				}
			}
		} else {
			MBCRawUploadUtil.log(logger,
					"     *** Found existing raw Series ID = " + rawSeriesID
							+ " - skipping");
			chkSumOK = true; // This will trigger a deletion since the file has
	...
	private static String createPSSDAssets(ServerClient.Connection cxn,
			File file, MRMetaData pm, String subjectCID, UserCredential cred,
		...
		// Look for MR raw STudy associated with this Patient
		// There is no mechanism to find a pre-existing Study as there is no
		// date we can
		// use in the meta-data to find one. So either the users chooses to
		// bundle
		...
		if (ops.bundle) {
			...
			if (ops.rawStudyCID == null) {
				ops.rawStudyCID = createRawStudy(cxn, pm, null, subjectCID,
						cred);
				MBCRawUploadUtil.log(logger,
						"     Created raw PSSD Study asset = "
								+ ops.rawStudyCID);
			} else {
				MBCRawUploadUtil.log(logger, "     PSSD Study asset = "
						+ ops.rawStudyCID);
			}
		} else {
			// Always make new Study
			ops.rawStudyCID = createRawStudy(cxn, pm, null, subjectCID, cred);
			MBCRawUploadUtil.log(logger, "     Created raw PSSD Study asset = "
					+ ops.rawStudyCID);
		}
	...
	private static Boolean createPSSDDataSet(ServerClient.Connection cxn,
			File file, MRMetaData pm, Options ops, PrintWriter logger)
			throws Throwable {

		MBCRawUploadUtil.log(logger, "   Uploading file");
		long tsize = FileUtils.sizeOf(file);
		MBCRawUploadUtil.log(logger,
				"      File size = " + FileUtils.byteCountToDisplaySize(tsize));
		String rawSeriesCID = createRawSeries(cxn, file, pm, null,
				ops.rawStudyCID, ops.expire);
		if (rawSeriesCID == null) {
			throw new Exception("Failed to create PSSD DataSet asset");
		}
		MBCRawUploadUtil.log(logger, "      Created raw PSSD DataSet asset = "
				+ rawSeriesCID);
		Boolean chkSumOK = false;
		if (ops.chksum) {
			MBCRawUploadUtil.log(logger, "        Validating checksum");
			...
			if (chkSumDisk.equalsIgnoreCase(chkSumAsset)) {
				...
				MBCRawUploadUtil.log(logger,
						"       Checksums do not match. Checksums are:");
				MBCRawUploadUtil.log(logger, "           Input file      = "
						+ chkSumDisk);
				MBCRawUploadUtil.log(logger, "           Mediaflux asset = "
						+ chkSumAsset);
				//
				MBCRawUploadUtil.log(logger,
						"           Destroyed Mediaflux asset");
	...
	private static void deleteFile(File file, PrintWriter logger)
			throws Throwable {
		try {
			FileUtils.forceDelete(file);
			MBCRawUploadUtil.log(logger, "     Successfully deleted file "
					+ file.getAbsolutePath());
		} catch (Exception e) {
			MBCRawUploadUtil.log(logger, "     *** Failed to delete file "
					+ file.getPath() + " with error " + e.getMessage());
		}
	}
	...
	/*
	 * private static String findRawStudy (ServerClient.Connection cxn,
	 * MRMetaData pm, String patientAssetID, String subjectCID) throws Throwable
	 * { // FInds by date only as there is no study 'UID'. Only UIDs for Series
	 * (data sets). XmlStringWriter w = new XmlStringWriter(); String query =
	 * null; if (subjectCID!=null) { query =
	 * "model='om.pssd.study' and cid starts with '" + subjectCID + "'";
	 * w.add("action", "get-cid"); } else { query =
	 * nameSpaceQuery(MF_NAMESPACE); query += " and (related to{had-by} (id=" +
	 * patientAssetID + "))"; } query += " and xpath(" + RAW_STUDY_DOC_TYPE +
	 * "/date)='" + DateUtil.formatDate(pm.getAcquisitionDateTime(), false,
	 * null) + "'"; w.add("where", query); XmlDoc.Element r =
	 * cxn.execute("asset.query", w.document()); if (r==null) return null; if
	 * (subjectCID!=null) { return r.value("cid"); } else { return
	 * r.value("id"); } }
	 */
	/**
	...
	private static String createRawStudy(ServerClient.Connection cxn,
		...
		if (subjectCID != null) {
			...
			w.push("related");
			w.add("to", new String[] { "relationship", "had-by" },
					patientAssetID);
			w.pop();
		...
		//
		// String date = DateUtil.formatDate(pm.getAcquisitionDateTime(), false,
		// null);
		// w.add("date", date);
		//
		w.push("ingest");
	...
	private static String findExMethod(ServerClient.Connection cxn,
		...
		if (mid == null) {
			throw new Exception("There is no Method for PSSD subject "
					+ subjectCID);
		}
		...
		if (exMethodCID == null) {
			throw new Exception(
					"There is no instantiated ex-method for PSSD subject "
							+ subjectCID);
		}
	...
	/**
	 * The only way we can find if the DataSet pre-exists is by its name. Not
	 * very robust. DaRIS data model only
	 * 
	 * @param cxn
	 * @param file
	 * @param sid
	 ...
	 * @throws Throwable
	 */
	private static String findRawSeries(ServerClient.Connection cxn, File file,
		...
		String query = null;
		query = "type='siemens-raw-mr/series' and model='om.pssd.dataset' and cid starts with '"
				+ sid
				+ "'"
				+ " and xpath(daris:pssd-object/name)='"
				+ name
	...
	private static String createRawSeries(ServerClient.Connection cxn,
		...
		if (rawStudyCID != null) {
			...
			w.add("namespace", MF_NAMESPACE);
			w.push("related");
			w.add("to", new String[] { "relationship", "container" },
					rawStudyID);
			w.pop();
		}

		/*
		 * w.push("meta"); w.push(RAW_SERIES_DOC_TYPE); //
		 * 
		 * String date = DateUtil.formatDate(pm.getAcquisitionDateTime(), true,
		 * false); w.add("date", date); w.add("modality", pm.getModality());
		 * w.add("description", pm.getDescription()); w.add("type",
		 * pm.getAcquisitionType()); w.add("series_number",
		 * pm.getSeriesNumber()); w.add("instance", pm.getInstanceNumber());
		 * w.add("uuid", pm.getUUID()); w.add("date-export",
		 * pm.getExportDateTime()); // if (expire) { Calendar c =
		 * Calendar.getInstance(); Date t = pm.getAcquisitionDateTime();
		 * c.setTime(t); c.add(Calendar.YEAR, 1); w.add("date-expire",
		 * c.getTime()); }
		 * 
		 * w.pop(); w.pop();
		 */
	...
	private static String getCheckSum(ServerClient.Connection cxn, String id,
	...
	private static void printHelp() {
		...
		System.out
				.println("   Uploads raw Siemens MR 'meas.dat' files to DaRIS/Mediaflux.  Assets should be uploaded");
		System.out
				.println("   to pre-existing Subjects with extant DICOM images.");
		System.out.println("");
		System.out.println("Usage:");
		System.out
				.println("   " + MBCMRUpload.class.getName() + " [options..]");
		System.out.println("");
		System.out.println("");
		System.out.println("Java Properties:");
		System.out
				.println("    -mf.host      [Required]: The name or IP address of the Mediaflux host.");
		System.out
				.println("    -mf.port      [Required]: The server port number.");
		System.out
				.println("    -mf.token     [Optional]: The security token to authenticate with (preferred).");
		System.out
				.println("    -mf.user      [Optional]: The logon user (if no token).");
		System.out
				.println("    -mf.domain    [Optional]: The logon domain (if no token).");
		System.out
				.println("    -mf.password  [Optional]: The logon user's (obfuscated) password (if no token).");
		System.out
				.println("    -mf.transport [Optional]: Required if the port number is non-standard.");
		System.out
				.println("                              One of [HTTP, HTTPS, TCPIP]. By default the");
		System.out
				.println("                              following transports are inferred from the port:");
		...
		System.out
				.println("   "
						+ PATH_ARG
						+ "          The path for the directory holding the data (all files matching the pattern 'meas*.dat' will be upload) or a single file. Default is "
						+ DEFAULT_SRC_PATH);
		System.out
				.println("   "
						+ FIND_ARG
						+ "   Method to find pre-existing subjects; one of 'name', 'id'(default) or 'name+id'");
		System.out.println("   " + NOLOG_ARG
				+ "        Disables writing any log file.");
		System.out
				.println("   "
						+ LOGPATH_ARG
						+ "       Specify the directory for log files to be written in. Defaults is "
						+ DEFAULT_LOGGER_PATH);
		System.out.println("   " + NOCHKSUM_ARG
				+ "     Disables check sum validation of uploaded file");
		System.out
				.println("   "
						+ NODELETE_ARG
						+ "     Disables the deletion of the input file after check sum validation");
		System.out
				.println("   "
						+ EXPIRE_ARG
						+ "        Specifies that meta-data is to be attached to the file with an expiry data of 1 year after acquisition");
		System.out
				.println("   "
						+ ID_ARG
						+ "            Specifies the PSSD data model (DaRIS) should be used and that this is the citeable ID that the Study should be associated with. Can be depth 3 (a Project), 4 (a Subject) or 6 (a Study).");
		System.out.println("   " + NIG_META_ARG);
		System.out
				.println("                  Specifies the NIG-domain meta-data should be added to the PSSD Subject");
		System.out.println("   " + DECRYPT_ARG
				+ "    Specifies the password should not be decrypted.");
		System.out
				.println("   "
						+ NOBUNDLE_ARG
						+ "     Specify that each file uploaded is to have its own parent Study. By default, all files uploaded are bundled under the same Study.");
		System.out
				.println("   "
						+ NOSKIP_ARG
						+ "       Do not attempt to find DataSets pre-existing (by name); if specified, then a new Study will always be created and all files uploaded to it");
		System.out.println("");
		System.out.println("");
	}
}
