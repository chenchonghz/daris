...
public class ScpSink implements DataSinkImpl {
    ...
    public static enum ParamDefn {
        ...
        PRIVATE_KEY("private-key",StringType.DEFAULT, "The user's private key for authenticating with the remote SSH server using pubkey authentication. It assumes the user's public key has been installed on the server, e.g. ~/.ssh/authorized_keys. If not specified, password must be specified."),
        PASSPHRASE("passphrase", StringType.DEFAULT, "The passphrase for the private-key. It is only required when the private-key argument is given and it is encrypted."),
