...
public class DicomImageGet extends ObjectMessage<DicomImage> {
	...
	public DicomImageGet(String assetId, int index, int frame, boolean lossless, double quality, double size) {
	...
	public DicomImageGet(String assetId, int index, int frame, boolean lossless) {

		this(assetId, index, frame, lossless, 0.8, 1.0);
	}

	public DicomImageGet(String assetId, int index, int frame) {

		this(assetId, index, frame, false, (float) 0.8, (float) 1.0);
	}

//	public DicomImageGet(String assetId, int index) {
//		this(assetId, index, -1, false, (float) 0.8, (float) 1.0);
//	}
//
//	public DicomImageGet(String assetId) {
//
//		this(assetId, 0, -1, false, (float) 0.8, (float) 1.0);
//	}
	...
	protected void messageServiceArgs(XmlWriter w) {
		...
		String idx = "" + _index;
		if (_frame >= 0) {
			w.add("id", new String[] { "idx", idx, "frame", String.valueOf(_frame) }, _assetId);
		} else {
	...
	protected DicomImage instantiate(XmlElement xe) throws Throwable {
		...
		if (xe != null) {
			return new DicomImage(_assetId, _index, _frame, null, _lossless);
		}
		return null;
	}
	...
	@Override
	public ObjectMessage<DicomImage> send(final ObjectMessageResponse<DicomImage> rh) {
		...
		RemoteServer.execute(messageServiceName(), w.document(), numberOfOutputs(), new ResponseHandler() {
			...
			public void processResponse(XmlElement xe, List<Output> outputs) {
				...
				if (numberOfOutputs() > 0) {
					rh.responded(new DicomImage(_assetId, _index, _frame, outputs.get(0).url(), _lossless));
				} else {
					rh.responded(null);
				}
			}
