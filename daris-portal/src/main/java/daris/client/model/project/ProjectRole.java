...
public enum ProjectRole {
    ...
    public static EnumerationType<ProjectRole> asEnumerationType() {
        ...
        List<EnumerationType.Value<ProjectRole>> evs = new Vector<EnumerationType.Value<ProjectRole>>(values().length);
        for (int i = 0; i < values().length; i++) {
            evs.add(new EnumerationType.Value<ProjectRole>(values()[i].toString(), values()[i].toString(), values()[i]));
        }
