...
public class ModifiedByFilter extends Filter {
    ...
    public ModifiedByFilter(String domain, String user) {
        _domain = domain;
        _user = user;
        _self = false;
    }

    public ModifiedByFilter() {
        _domain = null;
        _user = null;
        _self = true;
    }

    public ModifiedByFilter(XmlElement xe) {
        if (!_self) {
    ...
    public Filter copy() {
        if (self()) {
            return new ModifiedByFilter();
        } else {
            return new ModifiedByFilter(domain(), user());
        }
    }
