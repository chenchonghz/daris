...
public class DaRISLoginDialog implements LoginDialog, SessionHandler {
    ...
    public void show(String domain, String user, final Action action) {
        if (_showing) {
            return;
        }
        int width = 300;
        ...
        if (_icon != null) {
            if (_ipos == IconPosition.LEFT || _ipos == IconPosition.RIGHT) {
                width += _icon.width() + 8;
            } else if (_ipos == IconPosition.TOP || _ipos == IconPosition.BOTTOM) {
        ...
        if (domain == null) {
            _domainField = new Field<String>(new FieldDefinition("Domain", StringType.DEFAULT,
            ...
            _domainField = new Field<String>(new FieldDefinition("Domain", ConstantType.DEFAULT,
                    "The authentication domain", null, 1, 1));
        ...
        if (user == null) {
            _userField = new Field<Object>(new FieldDefinition(_userLabel, _userType, _userDesc, null, 1, 1));
            if (_user != null) {
            ...
            _userField = new Field<Object>(new FieldDefinition(_userLabel, ConstantType.DEFAULT, _userDesc, null, 1, 1));
        ...
        _passwordField = new Field<String>(new FieldDefinition("Password", PasswordType.DEFAULT,
        ...
        form2.setFontSize(9);
        form2.setLeft(80);
        _rememberDomainAndUserField = new Field<Boolean>(new FieldDefinition("Remember me",
                BooleanType.DEFAULT_TRUE_FALSE, null, null, 1, 1));
        _rememberDomainAndUserField.setInitialValue(_rememberDomainAndUser);
        _rememberDomainAndUserField.addListener(new FormItemListener<Boolean>() {
    ...
    private BaseWidget createIconGUI(BaseWidget gui) {
        ...
        arc.gui.gwt.widget.image.Image img = new arc.gui.gwt.widget.image.Image(_icon);

        if (_ipos == LoginDialog.IconPosition.TOP || _ipos == LoginDialog.IconPosition.BOTTOM) {
        ...
        if (_ipos == LoginDialog.IconPosition.LEFT || _ipos == LoginDialog.IconPosition.RIGHT) {
    ...
    private void doLogin(final Action action) {
        ...
        Session.logon(_domainField.valueAsString(), _userField.valueAsString(), _passwordField.valueAsString(),
                new ResponseHandler() {
    ...
    public void sessionCreated(boolean initial) {
        ...
        UserCookies.setDomain(_rememberDomainAndUser ? Session.domainName() : null);
