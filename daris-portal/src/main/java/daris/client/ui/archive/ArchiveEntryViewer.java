...
public class ArchiveEntryViewer implements InterfaceComponent {
	private static String[] _supportedImageExtensions = new String[] { "bmp", "tif", "tiff", "gif", "png", "jpg",
			"jpeg", "dcm" };

	private SimplePanel _sp;
	private ArchiveEntryCollectionRef _arc;
	private ArchiveEntry _entry;
	private boolean _mayContainViewableImage;

	public ArchiveEntryViewer(ArchiveEntryCollectionRef arc, boolean mayContainViewableImage) {
		_arc = arc;
		_mayContainViewableImage = mayContainViewableImage;
	...
	protected BaseWidget viewerWidgetFor(final ArchiveEntryCollectionRef arc, final ArchiveEntry entry) {
		if (_mayContainViewableImage && isSupportedImage(entry)) {
