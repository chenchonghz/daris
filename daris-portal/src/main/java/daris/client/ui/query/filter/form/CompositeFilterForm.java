...
public class CompositeFilterForm extends FilterForm<CompositeFilter> {
    ...
    public CompositeFilterForm(CompositeFilter filter, boolean editable, FilterTree filterTree) {
        ...
        DataSource<ListGridEntry<CompositeFilter.Member>> ds = new DataSource<ListGridEntry<Member>>() {
            ...
            @Override
            public void load(arc.gui.gwt.data.filter.Filter f, long start, long end,
                ...
                List<CompositeFilter.Member> fmembers = new ArrayList<CompositeFilter.Member>(members);
                if (f != null) {
                    for (Iterator<CompositeFilter.Member> it = fmembers.iterator(); it.hasNext();) {
                ...
                fmembers = start0 < total ? fmembers.subList(start0, end0) : null;
                ...
                for (int i = 0; i < fmembers.size(); i++) {
                    ...
                    ListGridEntry<CompositeFilter.Member> entry = new ListGridEntry<CompositeFilter.Member>(filter);
                    entry.set("index", i);
                ...
                lh.loaded(start0, end0, entries.size(), entries, DataLoadAction.REPLACE);
            }
        };
        ...
        if (editable()) {
            _grid.addColumnDefn("index", "", "delete this filter", new WidgetFormatter<Member, Integer>() {
        ...
        _grid.addColumnDefn("operator", "operator", null, new WidgetFormatter<Member, LogicOperator>() {

            @Override
            public BaseWidget format(Member member, LogicOperator op) {
                boolean first = filter().indexOf(member) == 0;
                if (!first) {
                    OperatorItem c = new OperatorItem(member, editable());
        ...
        _grid.addColumnDefn("negated", "negation", null, new WidgetFormatter<Member, Boolean>() {
        ...
        ListGridColumn<Filter> filterCol = _grid.addColumnDefn("filter", "filter", null,
                new WidgetFormatter<Member, Filter>() {

                    @Override
                    public BaseWidget format(Member context, Filter filter) {
                        FilterItem<?> item = null;
                        item = FilterItemFactory.createItem(CompositeFilterForm.this, filter, editable());
                        if (item != null) {
                            CompositeFilterForm.this.addMustBeValid(item);
                            /*
                             * Widget in list grid cell does not have its parent (ContainerWidget) set. Therefore, its
                             * window() can not be resolved. Here we manually set its window.
                             */
        ...
        _grid.setDropHandler(new DropHandler() {
            ...
            @Override
            public void drop(BaseWidget target, List<Object> objs, DropListener dl) {
        ...
        if (editable()) {
            ...
            treeLabel.setBackgroundImage(new LinearGradient(LinearGradient.Orientation.TOP_TO_BOTTOM,
    ...
    public Validity valid() {
                ...
                if (!iv.valid()) {
                    ...
                    if (v.valid()) {
                        v = new Validity() {
                            ...
                            @Override
                            public String reasonForIssue() {
                                return "sub-filter row " + idx + " is invalid: " + iv.reasonForIssue();
                            }
                        };
                    }
                }
        ...
        _statusIcon.show();
        _statusHTML.setHTML(v.valid() ? filter().toString() : v.reasonForIssue());
    ...
    protected void addMustBeValid(MustBeValid mbv, boolean saveRef) {
        if (saveRef) {
            _mbvs.add(mbv);
    ...
    public static CompositeFilterForm create(Filter filter, boolean editable, FilterTree filterTree) {
        if (filter instanceof ObjectCompositeFilter) {
            return new ObjectCompositeFilterForm((ObjectCompositeFilter) filter, editable, filterTree);
        } else {
            return new CompositeFilterForm(CompositeFilter.wrapIfItIsNotComposite(filter), editable, filterTree);
        }
    }
    ...
    public static CompositeFilterForm create(final HasComposite hasComposite) {
        ...
        if (composite instanceof ObjectCompositeFilter) {
            form = new ObjectCompositeFilterForm((ObjectCompositeFilter) composite, item.editable(),
                    item.form() == null ? null : item.form().filterTree(), false);
        } else {
            form = new CompositeFilterForm(composite, item.editable(), item.form() == null ? null : item.form()
                    .filterTree());
        }
