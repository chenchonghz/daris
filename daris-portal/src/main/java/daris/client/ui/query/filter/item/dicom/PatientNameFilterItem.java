...
import daris.client.model.query.filter.dicom.PatientNameFilter.NameType;
...
public class PatientNameFilterItem extends FilterItem<PatientNameFilter> {

    private HorizontalPanel _hp;
    private SimplePanel _formSP;
    private Form _form;

    public PatientNameFilterItem(CompositeFilterForm form, PatientNameFilter filter,
            boolean editable) {
        super(form, filter, editable);
        _hp = new HorizontalPanel();
        _hp.setHeight(22);

        HTML label = new HTML("dicom patient name");
    ...
    private void updateForm() {
        ...
        _form.setNumberOfColumns(filter().operator() == null
                || filter().operator().numberOfValues() < 1 ? 2 : 3);

        Field<NameType> typeField = new Field<NameType>(new FieldDefinition("type",
                new EnumerationType<NameType>(NameType.values()), null, null, 1, 1));
        typeField.setInitialValue(filter().nameType(), false);
        typeField.addListener(new FormItemListener<NameType>() {

            @Override
            public void itemValueChanged(FormItem<NameType> f) {
                filter().setNameType(f.value());
            }

            @Override
            public void itemPropertyChanged(FormItem<NameType> f, Property property) {
