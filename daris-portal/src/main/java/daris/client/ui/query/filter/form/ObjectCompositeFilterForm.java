...
public class ObjectCompositeFilterForm extends CompositeFilterForm {
	...
	public ObjectCompositeFilterForm(ObjectCompositeFilter filter,
			boolean editable) {
		this(filter, editable, new ObjectFilterTree(filter.project()), true);
	}

	public ObjectCompositeFilterForm(ObjectCompositeFilter filter,
			boolean editable, FilterTree filterTree) {
		this(filter, editable, filterTree, true);
	}

	public ObjectCompositeFilterForm(ObjectCompositeFilter filter,
			boolean editable, FilterTree filterTree, boolean canChangeProject) {
		super(filter, editable, filterTree == null ? new ObjectFilterTree(
				filter.project()) : filterTree);
		_vp = new VerticalPanel();
		_vp.fitToParent();
		_projectForm = new Form(canChangeProject ? FormEditMode.UPDATE
				: FormEditMode.READ_ONLY);
		final Field<DObjectRef> projectField = new Field<DObjectRef>(
				new FieldDefinition("Search in project",
						new EnumerationType<DObjectRef>(
								new ProjectEnumDataSource()), null, null, 1, 1));
		if (project() != null) {
			projectField.setInitialValue(project(), false);
		} else {
			new DObjectCollectionRef()
					.resolve(new CollectionResolveHandler<DObjectRef>() {

						@Override
						public void resolved(List<DObjectRef> projects)
								throws Throwable {
		...
		projectField.addListener(new FormItemListener<DObjectRef>() {
			...
			@Override
			public void itemPropertyChanged(FormItem<DObjectRef> f,
					Property property) {
		...
		addMustBeValid(_projectForm, true, false);
