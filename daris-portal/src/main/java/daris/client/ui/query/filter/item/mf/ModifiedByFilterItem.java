...
public class ModifiedByFilterItem extends FilterItem<ModifiedByFilter> {
    private HorizontalPanel _hp;
    private SimplePanel _formSP;
    private Form _form;

    public ModifiedByFilterItem(CompositeFilterForm cform, ModifiedByFilter filter, boolean editable) {
        super(cform, filter, editable);
        _hp = new HorizontalPanel();
        _hp.setHeight(22);

        HTML label = new HTML("modified by");
