...
public class SeriesProtocolFilterItem extends FilterItem<SeriesProtocolFilter> {

    private HorizontalPanel _hp;
    private SimplePanel _formSP;
    private Form _form;

    public SeriesProtocolFilterItem(CompositeFilterForm containerForm, SeriesProtocolFilter filter,
            boolean editable) {
        super(containerForm, filter, editable);
        _hp = new HorizontalPanel();
        _hp.setHeight(22);

        HTML label = new HTML("dicom series protocol");
