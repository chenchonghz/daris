...
public class DObjectBrowser extends ContainerWidget implements DTIReadyListener {
    ...
    public static final arc.gui.image.Image ICON_DICOM = new arc.gui.image.Image(
            Resource.INSTANCE.dicom24().getSafeUri().asString(), 16, 16);
    ...
    public static final arc.gui.image.Image ICON_HELP = new arc.gui.image.Image(
            Resource.INSTANCE.help16().getSafeUri().asString(), 16, 16);
    ...
    private MenuButton _importMenuButton;
    ...
    private DObjectBrowser() {
        ...
        darisMenu.add(new ActionEntry(ICON_ABOUT, "About DaRIS Portal",
                new Action() {
        ...
        darisMenu.add(new ActionEntry(ICON_ANNOUNCEMENT,
                "System Announcements...", new Action() {
        ...
        MenuButton darisMenuButton = ButtonUtil.createMenuButton(
                ICON_PREFERENCES, darisMenu.label(), darisMenu);
        _tb.add(darisMenuButton);

        Menu settingsMenu = new Menu("Settings");
        settingsMenu.add(new ActionEntry(ICON_DICOM,
                "DICOM Application Entities", new Action() {
        ...
        downloadMenu.add(new ActionEntry(ICON_ACTIVE,
                "Show active shopping cart...", new Action() {
                    ...
                    public void execute() {
                        // TODO: clean up
                        // ActiveShoppingCartDialog.instance().show(window(),
                        // 0.6, 0.5);
                        ShoppingCartDialog.get().showDialog(window(), true);
                    }
                }));
        downloadMenu.add(new ActionEntry(ICON_MANAGER,
                "Show all shopping carts...", new Action() {

                    @Override
                    public void execute() {
                        // TODO: clean up
                        // ShoppingCartManagerDialog.instance().show(window(),
                        // 0.6,
                        // 0.5);
        ...
        MenuButton downloadMenuButton = ButtonUtil.createMenuButton(
        ...
        _importMenuButton = ButtonUtil.createMenuButton(ICON_IMPORT,
                importMenu.label(), importMenu);
        _importMenuButton.setEnabled(arc.mf.client.dti.DTI.enabled());
        _tb.add(_importMenuButton);

        _searchMenuButton = ButtonUtil.createMenuButton(ICON_SEARCH, "Search",
                new Menu("Search"));
        ...
        transformMenu.add(new ActionEntry(ICON_TRANSFORM,
                "Show transform monitor", new Action() {
        ...
        final MenuButton transformMenuButton = ButtonUtil.createMenuButton(
                ICON_TRANSFORM, transformMenu.label(), transformMenu);

        new PackageExists("daris-transform")
                .send(new ObjectMessageResponse<Boolean>() {
                    ...
                    public void responded(Boolean r) {
                        if (r != null && r) {
                            ...
                            new PackageExists("Transform Framework")
                                    .send(new ObjectMessageResponse<Boolean>() {
        ...
        if (Plugin.isStandaloneApplication()) {
            Menu userMenu = new Menu(Plugin.isPluginEnvironment() ? "User"
            ...
            userMenu.add(new ActionEntry(ICON_SECURE_WALLET, "Secure Wallet",
                    new Action() {
            ...
            MenuButton userMenuButton = ButtonUtil.createMenuButton(ICON_USER,
        ...
        if (Plugin.isStandaloneApplication()) {
            ...
            _dtiStatusIcon.setEnabled(arc.mf.client.dti.DTI.enabled());
            _dtiStatus = new HTML("DTI: "
                    + (daris.client.DTI.installing() ? "loading..."
                            : (arc.mf.client.dti.DTI.enabled() ? "active"
                                    : "inactive")));
            ...
            arc.mf.client.dti.DTI.addReadyListener(this);
        }
        ...
        RepositoryRef.INSTANCE.resolve(new ObjectResolveHandler<DObject>() {
            ...
            public void resolved(DObject o) {
                _ov = new DObjectNavigator(new DObjectTree(
                        RepositoryRef.INSTANCE),
                        new ObjectNavigatorSelectionHandler() {

                            @Override
                            public void clickedObject(Node n, Object o,
                                    boolean readOnly) {
                    ...
                    public void openedObject(Node n, Object o) {
                        if (n != null && o != null) {
                            if (o instanceof DObjectRef
                                    && n instanceof DObjectTreeNode) {
                                objectOpened((DObjectTreeNode) n,
                                        (DObjectRef) o);
                            }
                        }
                    }
                    ...
                    public void closedObject(Node n, Object o) {
                        if (n != null && o != null) {
                            if (o instanceof DObjectRef
                                    && n instanceof DObjectTreeNode) {
                                objectClosed((DObjectTreeNode) n,
                                        (DObjectRef) o);
                            }
                        }
                    }
    ...
    private void updateActionMenu() {
        Menu menu = _selectedObject == null ? new Menu() : DObjectGUI.INSTANCE
                .actionMenu(window(), _selectedObject, null, false);
    ...
    private void updateSearchMenu() {
                ...
                "Search for DaRIS/PSSD objects.", new Action() {
                    ...
                    public void execute() {
                        ...
                        if (_selectedObject != null
                                && !_selectedObject.isRepository()) {
                            if (_selectedObject.isProject()) {
                                project = _selectedObject;
                            } else {
                                project = new DObjectRef(IDUtil
                                        .getProjectId(_selectedObject.id()));
                            }
                        }
        ...
        menu.add(new ActionEntry(ICON_LOAD, "Saved queries...",
                "Manage saved queries.", new Action() {
                        ...
                        SavedQueryBrowser.get().show(window(),
                                new ObjectResolveHandler<Query>() {
                                    ...
                                    public void resolved(Query query) {
                                        new SearchForm(query).show(window(),
                                                "Search for DaRIS objects...");
                                    }
                                });
                    }
                }));
    ...
    public void discard() {
        if (Plugin.isStandaloneApplication()) {
            arc.mf.client.dti.DTI.removeReadyListener(this);
        }
    }
    ...
    public void failed(String reason) {
        _dtiStatusIcon.setEnabled(arc.mf.client.dti.DTI.enabled());
        _dtiStatus.setHTML("DTI: failed");
        updateDTITooltip(true);
        _importMenuButton.disable();
    }

    @Override
    public void activated() {
        _dtiStatusIcon.setEnabled(arc.mf.client.dti.DTI.enabled());
        _dtiStatus.setHTML("DTI: activated");
        updateDTITooltip(false);
        _importMenuButton.enable();
    }

    @Override
    public void deactivated() {
        _dtiStatusIcon.setEnabled(arc.mf.client.dti.DTI.enabled());
        if (daris.client.DTI.installing()) {
            _dtiStatus.setHTML("DTI: loading...");
        } else {
            _dtiStatus.setHTML("DTI: deactivated");
        }
        updateDTITooltip(false);
        _importMenuButton.disable();
    }
