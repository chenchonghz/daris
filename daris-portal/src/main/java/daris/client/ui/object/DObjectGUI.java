...
public class DObjectGUI implements ObjectGUI {
    ...
    public static final Image ICON_CREATE = new Image(Resource.INSTANCE.add16().getSafeUri()
            .asString(), 16, 16);
    public static final Image ICON_EDIT = new Image(Resource.INSTANCE.edit16().getSafeUri()
            .asString(), 16, 16);
    public static final Image ICON_DICOM_SEND = new Image(Resource.INSTANCE.send16().getSafeUri()
            .asString(), 16, 16);
    public static final Image ICON_DICOM_INGEST = new Image(Resource.INSTANCE.upload16()
            .getSafeUri().asString(), 16, 16);
    public static final Image ICON_ADD_TO_SHOPPINGCART = new Image(Resource.INSTANCE
            .shoppingcart24().getSafeUri().asString(), 16, 16);
    ...
    public static final arc.gui.image.Image ICON_TAG = new arc.gui.image.Image(Resource.INSTANCE
            .tag16().getSafeUri().asString(), 16, 16);
                ...
                return new DropHandler() {
                    ...
                    public void drop(BaseWidget target, List<Object> data, DropListener dl) {
                        ...
                        if (type == DObject.Type.study) {
                            ...
                            m.add(new DerivedDataSetCreateAction(fs, new DObjectRef(IDUtil
                                    .getParentId(oo.id()), oo.proute(), false, false, -1), oo,
                                    target.window()));
                        }
                        if (m.entries() != null && !m.entries().isEmpty()) {
                            new ActionMenu(m)
                                    .showAt(target.absoluteLeft(), target.absoluteBottom());
                        }
    ...
    @Override
    public Menu actionMenu(final Window w, final Object o, SelectedObjectSet selected,
            boolean readOnly) {
         ...
         */
        if (ro.isStudy()) {
            menu.add(new ActionInterfaceEntry(ICON_CREATE, new PrimaryDataSetCreateAction(null, ro,
                    w)));
            menu.add(new ActionInterfaceEntry(ICON_CREATE, new DerivedDataSetCreateAction(null, ro,
                    null, w)));
        } else if (ro.isDataSet()) {
            menu.add(new ActionInterfaceEntry(ICON_CREATE, new DerivedDataSetCreateAction(null,
                    new DObjectRef(IDUtil.getParentId(ro.id()), ro.proute(), false, false, -1), ro,
                    w)));
        } else {
        ...
        if (ro.id() != null) {
            ...
            menu.add(new ActionEntry(ICON_ADD_TO_SHOPPINGCART, "Add to shopping cart",
                    new Action() {
             ...
             * search project
             */
            final DObjectRef project = ro.isProject() ? ro : new DObjectRef(IDUtil.getProjectId(ro
                    .id()));
            ...
            if (ro.isProject() || ro.isSubject() || ro.isExMethod() || ro.isStudy()
                ...
                final ActionEntry aeDestroy = new ActionEntry(ICON_DESTROY, label, description,
                        new Action() {

                            @Override
                            public void execute() {
                                Dialog.confirm("Deleting " + ro.referentTypeName() + " " + ro.id(),
                                        "Are you sure you want to delete " + ro.referentTypeName()
                                                + " " + ro.id() + " and all its descendants?",
                                        new ActionListener() {
             ...
             */
            if (ro.isProject() || ro.isSubject() || ro.isExMethod() || ro.isStudy()) {
                String label = "Delete processed datasets in " + ro.referentTypeName() + " "
                        + ro.id() + "...";
                String description = "Delete processed datasets in " + ro.referentTypeName() + " "
                        + ro.id();
                final ActionEntry aeDestroyProcessed = new ActionEntry(ICON_DESTROY_PROCESSED,
                        label, description, new Action() {

                            @Override
                            public void execute() {
                                Dialog.confirm(
                                        "Delete processed datasets in " + ro.referentTypeName()
                                                + " " + ro.id(),
             ...
             * dicom send
             */
            menu.add(new ActionInterfaceEntry(ICON_DICOM_SEND, new DicomSendAction((DObjectRef) o,
                    w)));

            /*
             * refresh (current object)
             */
            menu.add(new ActionEntry(ICON_RELOAD_GREEN, "Refresh " + ro.referentTypeName() + " "
                    + ro.id(), new Action() {
