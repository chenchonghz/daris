...
public class AttachmentGrid extends ListGrid<Attachment> implements DropHandler {
	...
	public AttachmentGrid(String cid) {
		...
		_aeAdd = new ActionEntry("Add attachment", new Action() {
			...
			@Override
			public void execute() {
				LocalFileSelectDialog dlg = new LocalFileSelectDialog(LocalFileSelectTarget.FILE, null, null,
						new LocalFileSelectDialog.FileSelectionHandler() {

							@Override
							public void fileSelected(LocalFile file) {
								new ObjectAttachmentAdd(_cid, file).send(new ObjectMessageResponse<Attachment>() {
		...
		_aeRemove = new ActionEntry("Remove attchment", new Action() {
			...
			public void execute() {
				...
				if (selections != null && !selections.isEmpty()) {
					new ObjectAttachmentRemove(_cid, selections).send(new ObjectMessageResponse<Boolean>() {
						@Override
		...
		_aeClear = new ActionEntry("Clear all attachments", new Action() {
			...
			@Override
			public void execute() {
				new ObjectAttachmentClear(_cid).send(new ObjectMessageResponse<Null>() {
	...
	public void drop(BaseWidget target, List<Object> objects, DropListener dl) {
		...
		for (Object object : objects) {
			new ObjectAttachmentAdd(_cid, (LocalFile) object).send(new ObjectMessageResponse<Attachment>() {
				@Override
				public void responded(Attachment r) {
