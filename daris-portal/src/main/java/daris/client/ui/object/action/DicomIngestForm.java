...
public class DicomIngestForm extends ValidatedInterfaceComponent implements AsynchronousAction {
    ...
    public DicomIngestForm(DObjectRef po, List<LocalFile> files) {
        ...
        _fileForm = new LocalFileForm(LocalFileSelectTarget.ANY, true, files);
    ...
    private Form createInterfaceForm() {
        ...
        Field<Boolean> anonymizeField = new Field<Boolean>(new FieldDefinition("anonymize",
                BooleanType.DEFAULT_TRUE_FALSE, "Anonymize the patient name.", null, 1, 1));
        anonymizeField.setValue(true);
        form.add(anonymizeField);

        Field<String> anonymizeElementsField = new Field<String>(new FieldDefinition("anonymize-elements",
                StringType.DEFAULT, "Indicates the DICOM elements to anonymize.", null, 0, 1));
        form.add(anonymizeElementsField);

        Field<String> engineField = new Field<String>(new FieldDefinition("engine", ConstantType.DEFAULT,
                "The type of storage engine to use to process the data.", null, 1, 1));
        engineField.setValue(DicomIngestTask.ENGINE_NIG_DICOM);
        form.add(engineField);

        FieldGroup argFieldGroup = new FieldGroup(new FieldDefinition("arg", DocType.DEFAULT,
                "An argument to pass through to the selected engine.", null, 0, Integer.MAX_VALUE));
        Field<String> argNameField = new Field<String>(new FieldDefinition("name", StringType.DEFAULT,
        ...
        Field<String> argValueField = new Field<String>(new FieldDefinition(null, StringType.DEFAULT,
        ...
        Field<String> typeField = new Field<String>(new FieldDefinition("type", new EnumerationType<String>(
                new TypeStringEnum()),
                "The MIME type of the input. If not specified, defined from the input stream itself.", null, 0, 1));
        form.add(typeField);

        Field<String> serviceField = new Field<String>(new FieldDefinition("service", StringType.DEFAULT,
                "The name of a service to be called for each study that is created/updated.", null, 0, 1));
                    ...
                    public void status(Timer t, DTITask task) {
                        if (task != null) {
                            ...
                            if (task.status() == DTITask.State.COMPLETED) {
                                MessageBox.display(MessageBox.Type.info, "DICOM ingest task",
                                        "DICOM data has been ingested successfully.", 3);
                            } else if (task.status() == DTITask.State.FAILED) {
                                Dialog.warn("Error", "DICOM ingest task " + task.id()
                                        + " failed with following error: <br/>" + task.errorAndStack());
                            } else if (task.status() == DTITask.State.FAILED_RETRY) {
                                MessageBox.display(MessageBox.Type.warning, "Warning", "DICOM ingest task " + task.id()
                                        + " failed with following error:<br/>" + task.errorAndStack()
                                        + "<br/>Retrying...", 3);
                            }
                        }
