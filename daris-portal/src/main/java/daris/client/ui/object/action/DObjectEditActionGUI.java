...
public class DObjectEditActionGUI extends ValidatedInterfaceComponent implements AsynchronousAction {

	private DObjectRef _o;
	private SimplePanel _gui;
	private boolean _checkedDuringPreconditions;

	public DObjectEditActionGUI(DObjectRef o, List<ActionPrecondition> duringPreconditions) {
		...
		if (duringPreconditions != null) {
			if (!duringPreconditions.isEmpty()) {
				ActionPreconditionInterface pci = new ActionPreconditionInterface(sa, duringPreconditions);
	...
	private void updateObjectDetails() {
		DObjectDetails details = DObjectDetails.detailsFor(_o.referent(), FormEditMode.UPDATE);
