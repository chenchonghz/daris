...
public class CanModifyPrecondition implements ActionPrecondition {

    private DObjectRef _o;

    public CanModifyPrecondition(DObjectRef o) {
    ...
    public void execute(final ActionPreconditionListener l) {
        ...
        new DObjectExists(_o).send(new ObjectMessageResponse<Boolean>() {

            @Override
            public void responded(Boolean exists) {
                if (exists) {
                    new CanModify(_o).send(new ObjectMessageResponse<Boolean>() {
                        ...
                        public void responded(Boolean canEdit) {
                            ...
                            if (canEdit) {
                                l.executed(ActionPreconditionOutcome.PASS,
                                        "You have privilege to edit object " + _o.id() + ".");
                            } else {
                                l.executed(ActionPreconditionOutcome.FAIL, "You do not have privilege to edit object "
                                        + _o.id() + ".");
                            }
                        }
                    });
                } else {
                    l.executed(ActionPreconditionOutcome.FAIL, "The " + _o.referentTypeName() + " " + _o.id()
