...
import daris.client.ui.exmethod.MethodAndStep;

public class Node implements GraphicsObject<MethodAndStep>, GraphicsImage.LoadHandler {
    ...
    protected void drawLabel(Graphics2D gc, double ox, double oy) {
        ...
        gc.setTextFont(FONT_WEIGHT + " " + FONT_STYLE + " " + FONT_VARIANT + " " + fontSize + "px " + FONT_FAMILY);
        gc.setTextBaseline("top");
        gc.setTextAlign("left");
        gc.setFillColour(Colours.BLACK);
        gc.fillText((int) ((_x + ox) * gc.width()), (int) ((_y + oy) * gc.height()), _name);
    }

    protected void drawBox(Graphics2D gc, Colour backgroundColour) {

        gc.setFillColour(backgroundColour);
        gc.fillRect((int) (_x * gc.width()), (int) (_y * gc.height()), (int) (_w * gc.width()),
                (int) (_h * gc.height()));

    }

    protected void drawBorder(Graphics2D gc, double borderWidth, Colour borderColour, LineStyle style) {
    ...
    protected void drawPorts(Graphics2D gc) {
        ...
        if (_inputs != null) {
            for (Port input : _inputs) {
                input.render(gc, false);
            }
        }
        if (_outputs != null) {
            for (Port output : _outputs) {
                output.render(gc, false);
            }
        }
    }
    ...
    @Override
    public MethodAndStep data() {
        return _mas;
    }

    @Override
    public void setData(MethodAndStep mas) {
        _mas = mas;
    }
