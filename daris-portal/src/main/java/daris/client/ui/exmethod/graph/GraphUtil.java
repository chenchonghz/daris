...
public class GraphUtil {
    ...
    public static double[][] ARROW = { { 0.0, 0.0 }, { -0.01, -0.004 },
            { -0.01, 0.004 } };
    ...
    public static void dashedLineTo(Graphics2D gc, double fromX, double fromY,
    ...
    public static void dashedLineTo(Graphics2D gc, double fromX, double fromY,
            double toX, double toY, double[] dashPattern) {
    ...
    public static void dashedLineTo(Graphics2D gc, int fromX, int fromY,
            int toX, int toY) {

        dashedLineTo(gc, fromX, fromY, toX, toY,
                toIntPattern(gc, DASH_PATTERN));
    }

    public static void dashedLineTo(Graphics2D gc, int fromX, int fromY,
            int toX, int toY, int[] dashPattern) {
        ...
        while (!((dx < 0 ? x <= toX : x >= toX)
            ...
            x = (int) (dx < 0 ? Math.max((double) toX, nx)
                    : Math.min((double) toX, nx));
            double ny = y + (Math.sin(angle) * dashLength);
            y = (int) (dy < 0 ? Math.max((double) toY, ny)
                    : Math.min((double) toY, ny));
            if (draw) {
                gc.lineTo(x, y);
            } else {
                gc.moveTo(x, y);
    ...
    public static void lineTo(Graphics2D gc, double fromX, double fromY,
            double toX, double toY, LineStyle style) {
    ...
    public static void lineTo(Graphics2D gc, int fromX, int fromY, int toX,
            int toY, LineStyle style) {
    ...
    public static void strokeDashedLine(Graphics2D gc, double fromX,
    ...
    public static void strokeDashedLine(Graphics2D gc, double fromX,
            double fromY, double toX, double toY, double[] dashPattern) {
    ...
    public static void strokeDashedLine(Graphics2D gc, int fromX, int fromY,
            int toX, int toY) {
    ...
    public static void strokeDashedRectangle(Graphics2D gc, double x, double y,
    ...
    public static void strokeDashedRectangle(Graphics2D gc, double x, double y,
            double width, double height, double[] dashPattern) {
    ...
    public static void strokeDashedRectangle(Graphics2D gc, int x, int y,
    ...
    public static void strokeDashedRectangle(Graphics2D gc, int x, int y,
            int width, int height, int[] dashPattern) {
    ...
    public static void strokeLine(Graphics2D gc, double fromX, double fromY,
            double toX, double toY, LineStyle style) {

        strokeLine(gc, (int) (fromX * gc.width()), (int) (fromY * gc.height()),
                (int) (toX * gc.width()), (int) (toY * gc.height()), style);
    }

    public static void strokeLine(Graphics2D gc, int fromX, int fromY, int toX,
            int toY, LineStyle style) {
    ...
    public static void strokeRectangle(Graphics2D gc, double x, double y,
            double width, double height, LineStyle style) {

        strokeRectangle(gc, (int) (x * gc.width()), (int) (y * gc.height()),
                (int) (width * gc.width()), (int) (height * gc.height()),
                style);
    }

    public static void strokeRectangle(Graphics2D gc, int x, int y, int width,
            int height, LineStyle style) {
    ...
    public static void strokeArrowLine(Graphics2D gc, int fromX, int fromY,
            int toX, int toY, LineStyle style, int[][] arrow,
            boolean doubleArrow) {
        ...
        if (doubleArrow) {
            ...
            drawArrow(gc,
                    translateArrow(rotateArrow(arrow, angle), fromX, fromY));
        }
    ...
    private static int[][] rotateArrow(int[][] arrow, double angle) {
        ...
        int[][] rv = new int[3][2];
        for (int i = 0; i < 3; i++) {
            rv[i][0] = (int) (arrow[i][0] * Math.cos(angle)
                    - arrow[i][1] * Math.sin(angle));
            rv[i][1] = (int) (arrow[i][0] * Math.sin(angle)
                    + arrow[i][1] * Math.cos(angle));
        }
        return rv;
    }

    public static void strokeArrowLine(Graphics2D gc, double fromX,
            double fromY, double toX, double toY, LineStyle style,
            double[][] arrow, boolean doubleArrow) {
