...
public class CTabPanel extends VerticalPanel implements HasBeforeSelectionHandlers<Integer>,
    ...
    public static final Colour TAB_COLOUR_DARK = new RGB(0xbb, 0xbb, 0xbb);

    public static final Colour TAB_COLOUR = new RGB(0xd9, 0xd9, 0xd9);

    public static final Colour TAB_COLOUR_SELECTED = new RGB(0x97, 0x97, 0x97);

    public static final Colour TAB_LIST_COLOUR = new RGB(0xe9, 0xe9, 0xe9);

    public static final Colour TAB_LIST_BORDER_COLOUR = new RGB(0x88, 0x88, 0x88);

    public static final String IMG_TICK = Resource.INSTANCE.tickBlue16().getSafeUri().asString();

    public static final String IMG_CLOSE = Resource.INSTANCE.tabClose16().getSafeUri().asString();

    public static final String IMG_CLOSE_HOVER = Resource.INSTANCE.tabCloseHover16().getSafeUri().asString();

    public static final String IMG_CLOSE_DISABLED = Resource.INSTANCE.tabCloseDisabled16().getSafeUri().asString();

    public static final String IMG_LEFT = Resource.INSTANCE.left16().getSafeUri().asString();

    public static final String IMG_LEFT_HOVER = Resource.INSTANCE.leftHover16().getSafeUri().asString();

    public static final String IMG_RIGHT = Resource.INSTANCE.right16().getSafeUri().asString();

    public static final String IMG_RIGHT_HOVER = Resource.INSTANCE.rightHover16().getSafeUri().asString();

    public static final String IMG_DOWN = Resource.INSTANCE.down16().getSafeUri().asString();

    public static final String IMG_DOWN_HOVER = Resource.INSTANCE.downHover16().getSafeUri().asString();

    public static final String IMG_CLOSE_ALL = Resource.INSTANCE.close16().getSafeUri().asString();

    public static final String IMG_CLOSE_ALL_HOVER = Resource.INSTANCE.closeHover16().getSafeUri().asString();
    ...
    public static class Tab extends HorizontalPanel {
        ...
        public Tab(String title, Object object, Widget content, boolean closable, boolean enabled) {

            this(null, title, object, content, closable, enabled);
        }

        protected Tab(CTabPanel tabPanel, String title, Object object, Widget content, boolean closable, boolean enabled) {
            ...
            _tabCloseImage.setBorder(1, RGBA.TRANSPARENT);
            ...
            // Style Properties
            setBackgroundImage(new LinearGradient(LinearGradient.Orientation.TOP_TO_BOTTOM, TAB_COLOUR, TAB_COLOUR_DARK));
        ...
        protected void onSelect() {
            ...
            setBackgroundImage(new LinearGradient(LinearGradient.Orientation.TOP_TO_BOTTOM, TAB_COLOUR_DARK,
                    TAB_COLOUR_SELECTED));
    ...
    private static class TabBar extends AbsolutePanel {
        ...
        private void initControlsAreaLeft() {
            ...
            _controlsAreaLeft.setBackgroundImage(new LinearGradient(LinearGradient.Orientation.TOP_TO_BOTTOM,
                    TAB_COLOUR_DARK, TAB_COLOUR_SELECTED));
            ...
            roamLeftBar.addMouseOverHandler(new MouseOverHandler() {
                ...
                public void onMouseOver(MouseOverEvent event) {
                    roamLeftBar.setBorder(1, BorderStyle.SOLID, new RGB(0x99, 0x99, 0x99));
                }
            });
            roamLeftBar.addMouseOutHandler(new MouseOutHandler() {
                @Override
                public void onMouseOut(MouseOutEvent event) {
                    roamLeftBar.setBorder(1, RGBA.TRANSPARENT);
                }
            });
        ...
        private void initControlsAreaRight() {
            ...
            _controlsAreaRight.setBackgroundImage(new LinearGradient(LinearGradient.Orientation.TOP_TO_BOTTOM,
                    TAB_COLOUR_DARK, TAB_COLOUR_SELECTED));
            _controlsAreaRight.setBorderLeft(1, BorderStyle.SOLID, new RGB(0xf0, 0xf0, 0xf0));
            ...
            roamRightBar.addMouseOverHandler(new MouseOverHandler() {
                @Override
                public void onMouseOver(MouseOverEvent event) {
                    roamRightBar.setBorder(1, BorderStyle.SOLID, new RGB(0x99, 0x99, 0x99));
                }
            });
            roamRightBar.addMouseOutHandler(new MouseOutHandler() {
                @Override
                public void onMouseOut(MouseOutEvent event) {
                    roamRightBar.setBorder(1, RGBA.TRANSPARENT);
                }
            });
            ...
            pickerBar.addMouseOverHandler(new MouseOverHandler() {
                @Override
                public void onMouseOver(MouseOverEvent event) {
                    pickerBar.setBorder(1, BorderStyle.SOLID, new RGB(0x99, 0x99, 0x99));
                }
            });
            pickerBar.addMouseOutHandler(new MouseOutHandler() {
                @Override
                public void onMouseOut(MouseOutEvent event) {
                    pickerBar.setBorder(1, RGBA.TRANSPARENT);
                }
            });
            ...
            closeAllBar.addMouseOverHandler(new MouseOverHandler() {
                @Override
                public void onMouseOver(MouseOverEvent event) {
                    closeAllBar.setBorder(1, BorderStyle.SOLID, new RGB(0x99, 0x99, 0x99));
                }
            });
            closeAllBar.addMouseOutHandler(new MouseOutHandler() {
                @Override
                public void onMouseOut(MouseOutEvent event) {
                    closeAllBar.setBorder(1, RGBA.TRANSPARENT);
                }
            });
        ...
        protected void showTabPicker() {
            ...
            for (Tab tab : _tabPanel.tabs()) {
                IconAndLabel label;
                if (tab.selected()) {
                    label = new IconAndLabel(new arc.gui.image.Image(IMG_TICK, 12, 12), tab.title());
                    label.setFontWeight(FontWeight.BOLD);
                } else {
                    label = new IconAndLabel(new arc.gui.image.Image(StandardImages.BLANK.path(), 12, 12), tab.title());
                ...
                public void over(Row r, MouseEvent event) {
                    ...
                    if (table != null) {
                        for (int i = 0; i < table.rowCount(); i++) {
                            ...
                            if (row != null) {
                                row.setBackgroundColour(TAB_LIST_COLOUR.toHTML());
                                // row.style().clearBackgroundColor();
                            }
                        }
                    }
                    if (r != null) {
                        r.setBackgroundColour(TAB_COLOUR_SELECTED.toHTML());
                    }
                }
            });
            ...
            _tabPicker.setPopupPositionAndShow(new PositionCallback() {
                ...
                public void setPosition(int offsetWidth, int offsetHeight) {
                    ...
                    _tabPicker.setPopupPosition(computeTabPickerLeft(), computeTabPickerTop());
                    _tabPicker.show();
                }
            });
        ...
        protected void doLayoutChildren() {
            ...
            int w1 = _tabsArea.widthWithMargins();
            int w = width();
            if (w1 > w) {
                if (_controlsAreaLeft.isAttached() && _controlsAreaRight.isAttached()) {
                    return;
                }
                if (!_controlsAreaLeft.isAttached()) {
                    add(_controlsAreaLeft);
                    _controlsAreaLeft.setLeft(0);
                    _tabsArea.setLeft(_controlsAreaLeft.widthWithMargins());
                }
        ...
        private int indexOfFirstVisibleTab() {
            ...
            int index = -1;
            int left = children().contains(_controlsAreaLeft) ? _controlsAreaLeft.widthWithMargins() : 0;
        ...
        private void moveLeft() {
            ...
            if (_tabsArea.absoluteRight() > _controlsAreaRight.absoluteLeft()) {
                ...
                if (indexOfFirstVisibleTab < _tabPanel.tabs().size() - 1) {
                    int widthOfFirstVisibleTab = _tabPanel.tabs().get(indexOfFirstVisibleTab)
                            .widthWithMargins();
                    _tabsArea.setLeft(_tabsArea.left() - widthOfFirstVisibleTab);
                }
            }
        }
        ...
        private void moveRight() {
            ...
            if (indexOfFirstVisibleTab > 0) {
                ...
                int widthOfLastInvisibleTab = _tabPanel.tabs().get(indexOfLastInvisibleTab)
                        .widthWithMargins();
        ...
        private boolean isTabHiddenLeft(Tab tab) {
            ...
            assert tab != null;
            int al = children().contains(_controlsAreaLeft) ? _controlsAreaLeft.absoluteRight() : absoluteLeft();
