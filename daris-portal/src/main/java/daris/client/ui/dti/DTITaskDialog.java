...
public class DTITaskDialog {
    ...
    public static final String ICON_ABORT = Resource.INSTANCE.abort24().getSafeUri().asString();
    public static final String ICON_DISMISS = Resource.INSTANCE.active24().getSafeUri().asString();
    public static final String ICON_BACKGROUND = Resource.INSTANCE.background24().getSafeUri().asString();
    ...
    public DTITaskDialog(DTITask task, Window owner) {
        ...
        cp.setHeight(20);
        cp.setWidth100();
        _duration = new HTML("Time elapsed: " + DateTime.durationAsString(task.duration(), false, false));
        ...
        _abortButton = ButtonUtil.createButton(ICON_ABORT, 16, 16, "Abort", null, false, new ClickHandler() {
        ...
        _backgroundButton = ButtonUtil.createButton(ICON_BACKGROUND, 16, 16, "Background", null, false,
                new ClickHandler() {
        ...
        _dismissButton = ButtonUtil.createButton(ICON_DISMISS, 16, 16, "Dismiss", null, false, new ClickHandler() {
        ...
        _task.monitor(1000, false, new DTITaskStatusHandler<DTITask>() {
            ...
            public void status(Timer t, DTITask task) {
                ...
                _status.setHTML("Status: " + task.status().toString().toLowerCase());
                _duration.setHTML("Time elapsed: " + DateTime.durationAsString(task.duration(), false, false));
                double progress = task.progress() <= 0.0 ? 0.0 : (task.progress() > 100 ? 100 : task.progress());
                _pb.setProgress(progress <= 0.0 ? 0.0 : progress / 100);
    ...
    private void updateButtons(DTITask task) {
        ...
        if (task.finished()) {
            ...
            if (status == DTITask.State.INITIAL || status == DTITask.State.SUBMITTED || status == DTITask.State.RUNNING
