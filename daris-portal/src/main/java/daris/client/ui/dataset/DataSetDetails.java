...
//            setTab(TAB_NAME_DICOM, TAB_DESC_DICOM, new DicomImageNavigator(dds.assetId(), dds.size()));
            // @formatter:on
            setTab(TAB_NAME_DICOM, TAB_DESC_DICOM, new DicomSeriesViewer(dds.assetId(), dds.size(),
    ...
    private void updateNiftiTab() {
        ...
        if (ds.data() != null
                && (MimeTypes.NIFTI_SERIES.equals(ds.mimeType()) || MimeTypes.NIFTI_SERIES.equals(ds.data()
                        .logicalMimeType()))) {
            ...
            Session.execute("package.exists", "<package>nig-analyzer</package>", 0, new ServiceResponseHandler() {
                ...
                public void processResponse(XmlElement xe, List<Output> outputs) throws Throwable {
                    ...
                    if (exists) {
                        setTab(TAB_NAME_NIFTI, TAB_DESC_NIFTI, new NiftiSeriesViewer(ds, false).widget());
                    } else {
                        setTab(TAB_NAME_NIFTI, TAB_DESC_NIFTI, new NiftiSeriesViewer(ds, true).widget());
                    }
                }
            });
        }
    ...
    protected void addToInterfaceForm(Form interfaceForm) {
        ...
        if (dso instanceof DerivedDataSet) {
            ...
            //
            Field<Boolean> anonymizedField = new Field<Boolean>(new FieldDefinition("anonymized",
                    BooleanType.DEFAULT_TRUE_FALSE, null, null, 0, 1));
            ...
            if (inputs != null && !inputs.isEmpty()) {
                for (DerivedDataSet.Input input : inputs) {
                    FieldGroup fgInput = new FieldGroup(new FieldDefinition("input", ConstantType.DEFAULT, null, null,
                            1, 1));
                    Field<String> inputVidField = new Field<String>(new FieldDefinition("vid", ConstantType.DEFAULT,
                            null, null, 1, 1));
                    inputVidField.setInitialValue(input.vid(), false);
                    Field<String> inputIdField = new Field<String>(new FieldDefinition(null, ConstantType.DEFAULT,
                            null, null, 1, 1));
        ...
        if (dso.transform() != null) {
            ...
            FieldGroup fgTransform = new FieldGroup(new FieldDefinition("transform", DocType.DEFAULT, null, null, 0, 1));
            ...
            if (dso.transform().software() != null) {
                for (Transform.Software s : dso.transform().software()) {
                    FieldGroup fgSoftware = new FieldGroup(new FieldDefinition("software", DocType.DEFAULT, null, null,
                            0, 1));
                    // software name
                    ...
                    if (s.commands() != null) {
                        for (Transform.Software.Command cmd : s.commands()) {
                            ...
                            // command name
                            Field<String> cnameField = new Field<String>(new FieldDefinition("name",
                                    StringType.DEFAULT, "The name of the command.", "The name of the command.", 1, 1));
                            ...
                            if (cmd.hasArgs()) {
                                for (String argName : cmd.args().keySet()) {
                                    ...
                                    // argument name
                                    Field<String> cargNameField = new Field<String>(new FieldDefinition("name",
                                            StringType.DEFAULT, "The name of argument.", "The name of the argument.",
                                            1, 1));
                                    cargNameField.setValue(argName);
                                    fgArgument.add(cargNameField);

                                    // argument value
                                    Field<String> cargValueField = new Field<String>(new FieldDefinition("value",
                                            StringType.DEFAULT, "The value of argument.", "The value of the argument.",
                                            1, 1));
