...
public class UserEnumerationDataSource implements
		DynamicEnumerationDataSource<UserRef> {
	...
	public void exists(final String value,
		...
		String user = value == null ? null : (value.indexOf(':') == -1 ? value
		...
		DomainRef domain = (_domain.authority() != null && _domain.authority()
				.name() == null) ? new DomainRef(null, _domain.name(),
				Domain.Type.LOCAL, null) : _domain;
	...
	public void retrieve(String prefix, final long start, final long end,
		...
		final String userPrefix = prefix == null ? null
				: (prefix.indexOf(':') == -1 ? prefix : prefix.substring(prefix
						.indexOf(':') + 1));
		if (_domain == null) {
		...
		new UserCollectionRef(_domain, new UserFilter() {
			...
			public FilterType type() {
				return UserFilter.FilterType.USER;
			}
			...
			public void resolved(List<UserRef> users) throws Throwable {
				...
				for (UserRef user : users) {
					values.add(new Value<UserRef>(user.name(), user.name(),
							user));
				}
