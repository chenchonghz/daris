...
public class DicomMF {
	...
	public static void main(String[] args) throws Throwable {
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equalsIgnoreCase("-help")) {
				...
				if (++i == args.length) {
					System.err.println("DicomMF: error: -tmp option must specify a String");
					System.exit(1);
				}
				tmpDir = args[i];
			} else {
		...
		Dicom.uploadMF(cxn, id, check, clean, dir, tmpDir, metaSetService, anonName);
	}
	...
	private static void printHelp (PrintStream os) {
		...
		os.println("   -tmp        Specify the parent temporary directory for packing files into an archive for upload. Default is /tmp");
		os.println("");
