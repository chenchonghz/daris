...
public class ExecuteCLI {
    ...
    private static class Options {
        ...
        public Options() {
            try {
                ...
                setApp(System.getProperty("mf.app"));
        ...
        public void setAuth(String auth) throws Throwable {
            ...
            throw new ArgumentParseException("Invalid option mf.auth: " + auth
    ...
    private static XmlDoc.Element execute(String host, int port,
        ...
        try {
            ...
            if (token != null) {
                if (app != null) {
                    cxn.connectWithToken(app, token);
                } else {
    ...
    private static XmlDoc.Element execute(String host, int port,
        ...
        try {
            cxn = server.open();
            if (app != null) {
                cxn.connect(app, domain, user, password);
            } else {
    ...
    private static void showHelp(Command cmd) {
        if (cmd == null) {
            ...
            System.out.println(
                    "    --mf.auth <domain,user,password> The Mediaflux user authentication details, it is in the form of domain,user,password. For execute command only.");
            System.out.println(
                    "    --mf.output <xml|shell>          The output format. Can be xml or shell. For execute command only.");
            System.out.println("");
