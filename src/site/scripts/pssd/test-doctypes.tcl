# Use with either:
# asset.set  :id 1530 :meta -action add < :nig-daris:pssd-human-contact-test2 < :contact < :name < :first Howard :last Hughes > :relationship magnate >  > >

proc createDocType-test {} {
	asset.doc.type.update \
            :create true \
            :type nig-daris:pssd-human-contact-test \
            :label "Human contact" \
            ...
            :definition < \
             :element -name contact -type document -min-occurs 0 -max-occurs infinity < \
                ...
                :element -name name -type document -min-occurs 1 -max-occurs 1  < \
                 ...
                 > \
               :element -name relationship -type enumeration  -dictionary nig-daris:pssd-human-relationship \
...
proc destroyDocType-pssd-human-contact-test { force } {
	...
	if { [xvalue exists [asset.doc.type.exists :type nig-daris:pssd-human-contact-test]] == "true" } {
		asset.doc.type.destroy :type nig-daris:pssd-human-contact-test :force $force
	}
...
proc createDocType-test2 {} {
	asset.doc.type.update \
            :create true \
            :type nig-daris:pssd-human-contact-test2 \
...
proc destroyDocType-pssd-human-contact-test2 { force } {
	...
	if { [xvalue exists [asset.doc.type.exists :type nig-daris:pssd-human-contact-test2]] == "true" } {
		asset.doc.type.destroy :type nig-daris:pssd-human-contact-test2 :force $force
	}
