...
#
   set r [ om.pssd.subject.create \
     ...
     :method $method_id \
     :public < \
       :nig-daris:pssd-subject < \
          :type animal \
          :control $control \
       > \
       :nig-daris:pssd-animal-subject <\
          :species mouse \
          :age -units days $age \
          :gender $gender \
        > \
       :nig-daris:pssd-animal-disease <\
          :disease  "Experimental Autoimmune Encephalomyelitis (EAE)" \
        > \
       :nig-daris:pssd-animal-genetics < \
          :strain <\
             :name C57Black \
             :fraction 1.0 \
          >\
       > \
       :nig-daris:pssd-identity < \
...
#      
   om.pssd.ex-method.subject.step.update \
      ...
      :state complete \
      :ps-meta <\
         :nig-daris:pssd-recovery < :method "warming light" >\
      >
#
   om.pssd.ex-method.subject.step.update \
      :id $mid \
...
#
   om.pssd.ex-method.subject.step.update \
      ...
      :state complete \
      :ps-meta <\
         :nig-daris:pssd-EAE-optic-nerve-removal <\
...
# Method/step study meta-data (no PSSD function; need to write om.pssd.ex-method.study.step.update)
    ...
    asset.set :cid $omid \
        :meta <\
           :nig-daris:pssd-EAE-microscopy -action add -ns $ns  <\
              :magnification $mag \
           >\
        >\
...
# Step 3 branch 1
   ...
   om.pssd.ex-method.subject.step.update \
      ...
      :state complete \
      :ps-meta <\
         :nig-daris:pssd-EAE-optic-nerve-section <\
            :thickness 0.5 \
          >\
      >
#
