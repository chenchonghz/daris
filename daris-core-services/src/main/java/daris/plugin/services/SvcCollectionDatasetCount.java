...
public class SvcCollectionDatasetCount extends PluginService {
    public static final String SERVICE_NAME = "daris.collection.dataset.count";

    private Interface _defn;

    public SvcCollectionDatasetCount() throws Throwable {
    ...
    public String description() {
        ...
        return "Count the number of datasets in the specified DaRIS collection.";
    }
    ...
    public void execute(Element args, Inputs inputs, Outputs outputs, XmlWriter w) throws Throwable {
        ...
        if (cid != null && where != null) {
            throw new IllegalArgumentException("Both cid and where argument are specified. Expecting only one.");
        }
        int count = 0;
        if (cid != null) {
            count = countDatasets(executor(), cid);
        } else {
            count = countDatasets(where, executor());
        }
        w.add("value", count);
    }
    ...
    public static int countDatasets(ServiceExecutor executor, String cid) throws Throwable {
        if (cid != null) {
            StringBuilder sb = new StringBuilder("cid='").append(cid).append("' or cid starts with '").append(cid)
                    .append("'");
            return countDatasets(sb.toString(), executor);
        } else {
            return countDatasets(null, executor);
        }
    }

    public static int countDatasets(String where, ServiceExecutor executor) throws Throwable {
        StringBuilder sb = new StringBuilder();
        if (where != null) {
            sb.append("(").append(where).append(")").append(" and ");
        }
        sb.append("(asset has content)").append(" and (model='om.pssd.dataset')");
