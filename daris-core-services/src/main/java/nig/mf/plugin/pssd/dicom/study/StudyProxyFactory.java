...
public class StudyProxyFactory {
	 ...
	 * @throws Throwable
	 */
	public static StudyProxy createStudyProxy(ServiceExecutor executor,
			DicomEngineContext ec, String studyUID, DataElementMap dem,
		...
		if (nextType != null) {
			if (nextType.equals(DicomAssetEngineConstants.PSS_ENGINE_NAME)) {
				DicomAssetEngine pss = ec
						.engine(DicomAssetEngineConstants.PSS_ENGINE_NAME);
				if (pss == null) {
					String msg = "Failed to create dicom engine: "
							+ NIGDicomAssetEngine.TYPE_NAME;
					Exception ex = new Exception(msg);
					DicomLog.error(msg);
					throw ex;
				}
				DicomLog.warn("Fall through to dicom engine: "
						+ DicomAssetEngineConstants.PSS_ENGINE_NAME);
	...
	private static StudyProxy createPSSDStudy(ServiceExecutor executor,
			String studyUID, DataElementMap dem, DicomIngestControls ic)
					throws Throwable {
		...
		if (cms == null || (cms != null && cms.cid() == null)) {
			DicomLog.warn(
					"No citable id extracted from dicom metadata header.");
			...
			cms = SubjectHandler.findSubjectByDetail(executor,
					ic.findSubjectMethod(), studyMeta, null);
		...
		DicomLog.info("StudyProxyFactory::createPSSDStudy:  cid_step = "
				+ cms.toString());
		...
		try {
			...
			DicomLog.warn(NIGDicomAssetEngine.TYPE_NAME
					+ " engine encountered an error: " + t.getMessage());
			return null;
		}
		...
		if (!ps.allowAccessToProject(modelUser, projectId)) {
			...
			// one
			String message = sendDeniedProjectMessage(executor, ps, modelUser,
					projectId);
		...
		boolean exists = nig.mf.plugin.util.AssetUtil.exists(executor, cid,
				true);
		try {
			...
			if (!exists && nig.mf.pssd.CiteableIdUtil.isSubjectId(cid)) {
				...
				if (ic.autoSubjectCreate()) {
					...
					String t = SubjectHandler.handleSubjectCreation(executor,
							projectId, cid, ic, studyMeta);
					if (t == null) {
					...
					if (projectId != null) {
						String subject = "DaRIS "
								+ NIGDicomAssetEngine.TYPE_NAME
								+ " engine: subject with citable identifier "
								+ cid + " does not exist";
						String msg = NIGDicomAssetEngine.TYPE_NAME
								+ " engine: subject " + cid
						...
						DicomLog.warn(msg);
						MailHandler.sendAdminMessage(executor, projectId,
								subject, msg);
					}
					return null;
				}
			...
			if (!exists) {
				DicomLog.warn(
						"Object with citable id " + cid + " is not found.");
				return null;
			}
			...
			dm.add("id", cid);
			XmlDoc.Element r = executor.execute("om.pssd.object.type",
					dm.root());
			String pssdType = r.stringValue("type", "unknown");
			if (pssdType.equals("unknown")) {
				DicomLog.warn("Asset with citable id " + cid
						+ " is not a DaRIS PSSD object.");
				return null;
			}
			...
			if (pssdType.equals("project")) {
				if (ic.autoSubjectCreate()) {
					...
					// We first try to find the Subject by name etc in this project.
					// We can use DICOM mf-dicom-patient or mf-dicom-patient-encrypted
					...
					if (cms == null) {
						DicomLog.info("No subject found in project " + cid
								+ ". Trying to auto-create subject.");
						cid = SubjectHandler.handleSubjectCreation(executor,
								projectId, null, ic, studyMeta);
						if (cid == null) {
			...
			// Stick on the standard mf-dicom-patient of mf-dicom-patient-encrypted meta-data in private. We
			// do this
			...
			if (pssdType.equalsIgnoreCase("subject")) {
				if (writeDICOMPatient) {
					SubjectHandler.addSubjectDICOMMetaData(executor, ic, cid,
							studyMeta);
				}
				
				// Sanity checks
				SubjectHandler.compareSubjectDICOMMetaData(executor, cid,
						studyMeta, false);
				SubjectHandler.compareSubjectDICOMMetaData(executor, cid,
						studyMeta, true);
				return new PSSDStudyProxy(nameSpace, studyUID, cid, null,
						cms.methodStep(), null, studyMeta, domainMetaService,
						ignoreModality, dropSR);
			}

			if (pssdType.equalsIgnoreCase("ex-method")) {
				String subjectCid = CiteableIdUtil.getParentId(cid);
				if (writeDICOMPatient) {
					SubjectHandler.addSubjectDICOMMetaData(executor, ic,
							subjectCid, studyMeta);
				}
				
				// Snaity checks
				SubjectHandler.compareSubjectDICOMMetaData(executor, subjectCid,
						studyMeta, false);
				SubjectHandler.compareSubjectDICOMMetaData(executor, subjectCid,
						studyMeta, true);
				//
				return new PSSDStudyProxy(nameSpace, studyUID, subjectCid, cid,
						cms.methodStep(), null, studyMeta, domainMetaService,
						ignoreModality, dropSR);
			}
			...
			if (pssdType.equalsIgnoreCase("study")) {
				...
				SubjectHandler.compareSubjectDICOMMetaData(executor, subjectCid,
						studyMeta, true);
				//
				return new PSSDStudyProxy(nameSpace, studyUID, subjectCid,
						exMethodCid, cms.methodStep(), cid, studyMeta,
			...
			if (projectId != null) {
				String subject = "DaRIS " + NIGDicomAssetEngine.TYPE_NAME
						+ " engine: unhandled citeable id ";
				String msg = NIGDicomAssetEngine.TYPE_NAME
						+ " engine: the citeable id " + cid
						+ " does not represent an DaRIS PSSD object "
						+ NIGDicomAssetEngine.TYPE_NAME
	 ...
	 * @throws Throwable
	 */
	private static CIDAndMethodStep extractCiteableID(ServiceExecutor executor,
		...
		if (ic.citableID() != null) {
			...
			if (CiteableIdUtil.isCiteableId(sid)) {
				if (CiteableIdUtil.getIdDepth(sid) >= CiteableIdUtil
						.projectDepth()) {
					cms.setCID(sid);
					return cms;
				} else {
		...
		for (int i = 0; i < ic.cidElements().length; i++) {
			cms = extractID(ic.cidElements()[i], dem, ic.ignoreNonDigits(),
					ic.ignoreBeforeLastDelim(), ic.ignoreAfterLastDelim());
			...
			if (cms != null) {
				...
				if (sid != null) {
					...
					boolean sidOK = CiteableIdUtil.isCiteableId(sid)
							&& CiteableIdUtil.getIdDepth(sid) >= CiteableIdUtil
							.projectDepth();
	 ...
	 */
	static private CIDAndMethodStep findProjectConfiguredForProjectFind(
			ServiceExecutor executor, StudyMetadata sm) throws Throwable {
		XmlDocMaker dm = new XmlDocMaker("args");
		...
		if (projects != null) {
			for (XmlDoc.Element project : projects) {
				...
				Collection<XmlDoc.Element> dicoms = project
						.elements("meta/daris:pssd-dicom-ingest/project/dicom");
				if (dicoms != null) {
					for (XmlDoc.Element dicom : dicoms) {
						...
						if (dicomType.equals("protocol_name")) {
							...
							throw new Exception(
									"Unhandled value for daris:pssd-ingest/project/dicom attribute 'type':"
											+ dicomType);
						}
	 ...
	 * @throws Throwable
	 */
	private static CIDAndMethodStep extractID(int ele, DataElementMap dem,
			boolean ignoreLeadingNonDigits, String ignoreBeforeLastDelim,
			String ignoreAfterLastDelim) throws Throwable {
		switch (ele) {
		case DicomIngestControls.ID_NONE:
			return null;

		case DicomIngestControls.ID_BY_PATIENT_FULL_NAME: {
			DicomPersonName pn = (DicomPersonName) dem
					.valueOf(DicomElements.PATIENT_NAME);
			if (pn == null)
				return null;
			...
			if (ignoreAfterLastDelim != null)
				...
				.removeAfterLastDelim(fullName, ignoreAfterLastDelim);
			if (ignoreBeforeLastDelim != null)
				fullName = nig.mf.pssd.CiteableIdUtil
				.removeBeforeLastDelim(fullName, ignoreBeforeLastDelim);
			//
			if (ignoreLeadingNonDigits)
				fullName = nig.mf.pssd.CiteableIdUtil
				.removeLeadingNonDigits(fullName);
		...
		case DicomIngestControls.ID_BY_PATIENT_LAST_NAME: {
			...
			if (ignoreAfterLastDelim != null)
				...
				.removeAfterLastDelim(lastName, ignoreAfterLastDelim);
			if (ignoreBeforeLastDelim != null)
				lastName = nig.mf.pssd.CiteableIdUtil
				.removeBeforeLastDelim(lastName, ignoreBeforeLastDelim);
			//
			if (ignoreLeadingNonDigits)
				lastName = nig.mf.pssd.CiteableIdUtil
				.removeLeadingNonDigits(lastName);
		...
		case DicomIngestControls.ID_BY_PATIENT_FIRST_NAME: {
			...
			if (ignoreAfterLastDelim != null)
				...
				.removeAfterLastDelim(firstName, ignoreAfterLastDelim);
			if (ignoreBeforeLastDelim != null)
				firstName = nig.mf.pssd.CiteableIdUtil.removeBeforeLastDelim(
						firstName, ignoreBeforeLastDelim);
			//
			if (ignoreLeadingNonDigits)
				firstName = nig.mf.pssd.CiteableIdUtil
				.removeLeadingNonDigits(firstName);
			if (firstName == null)
				return null;
		...
		case DicomIngestControls.ID_BY_PATIENT_ID: {
			...
			if (ignoreAfterLastDelim != null)
				id = nig.mf.pssd.CiteableIdUtil.removeAfterLastDelim(id,
						ignoreAfterLastDelim);
			if (ignoreBeforeLastDelim != null)
				id = nig.mf.pssd.CiteableIdUtil.removeBeforeLastDelim(id,
						ignoreBeforeLastDelim);
			//
			if (ignoreLeadingNonDigits)
		...
		case DicomIngestControls.ID_BY_REFERRING_PHYSICIAN_NAME: {
			String id = dem
					.stringValue(DicomElements.REFERRING_PHYSICIANS_NAME);
			if (id == null)
				return null;
		...
		case DicomIngestControls.ID_BY_REFERRING_PHYSICIAN_PHONE: {
			String id = dem
					.stringValue(DicomElements.REFERRING_PHYSICIANS_PHONE);
			if (id == null)
				return null;
		...
		case DicomIngestControls.ID_BY_PERFORMING_PHYSICIAN: {
			String id = dem
					.stringValue(DicomElements.PERFORMING_PHYSICIANS_NAME);
			if (id == null)
				return null;
	...
	private static CIDAndMethodStep extractCIDAndStep(String str)
			throws Throwable {
		if (str == null)
			return null;
		CIDAndMethodStep cms = new CIDAndMethodStep();
	...
	private static void sendWrongServerMessage(ServiceExecutor executor,
		...
		String message = "Dear sender of DICOM data \n You appear to have sent data with CID "
				+ cid + "\n to the wrong DaRIS server (which has  CID root "
				+ cidRoot + ")\n" + "The name of the server you sent to is "
				+ name + "\n" + "The organization of the server you sent to is "
				+ org + "\n Regards DaRIS...";
		sendMessageToProxyUser(executor, subject, message);
	}

	private static String sendDeniedProjectMessage(ServiceExecutor executor,
			DICOMProjectSelector ps, String modelUser, String cid)
					throws Throwable {
		String cidRoot = nig.mf.pssd.CiteableIdUtil.getRootParentId(cid);
		XmlDoc.Element r = executor.execute("server.identity");
		String name = r.stringValue("server/name", "Unknown");
		String org = r.stringValue("server/organization", "Unknown");
		String subject = "DICOM proxy user (" + modelUser
				+ ") denied access to project " + cid;
		String message = "Dear sender of DICOM data \n You have sent data with CID "
				+ cid + "\n to the DaRIS server with CID root " + cidRoot
	...
	private static void sendMessageToProxyUser(ServiceExecutor executor,
