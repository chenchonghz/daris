...
public class SvcUserDescribe extends PluginService {
	...
	// TODO: The interface needs to be rationalized so that specific role
	// members as well as
	// user members can be described. It will list all role members at present
	// if type=role
	...
	public SvcUserDescribe() {
		...
		_defn.add(new Interface.Element("user", StringType.DEFAULT,
				"The user in the authentication domain to list. Defaults to all", 0, 1));
		_defn.add(new Interface.Element("ignore-system", BooleanType.DEFAULT,
				"Ignore SYSTEM domain users ?  Defaults to true.", 0, 1));
		_defn.add(new Interface.Element("type", StringType.DEFAULT,
				"The type of user, 'user' (default) or 'role' (i.e. roles that may be project members) to list", 0, 1));
		_defn.add(new Interface.Element("list-projects", BooleanType.DEFAULT,
				"List the (local only) projects to which the user(s) have access?", 0, 1));
	}
	...
	/*
	 * public int executeMode() { return EXECUTE_DISTRIBUTED_ALL; }
	 */
	...
	public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w) throws Throwable {
		...
		if (userType.equals("role")) {
			...
			// Find the roles that have been registered for use as project
			// role-members
	...
	private void describe(ServiceExecutor executor, XmlWriter w, XmlDoc.Element ue, boolean projects,
			boolean ignoreSystem) throws Throwable {

		// Format
		Authority authority = Authority.instantiate(ue.stringValue("@authority"), ue.stringValue("@protocol"));
		String domain = ue.value("@domain");
		if (domain.equalsIgnoreCase("system") && ignoreSystem)
			return;
		//
		String user = ue.value("@user");
		String id = ue.value("@id");
		if (authority != null) {
			if (authority.protocol() != null) {
				w.push("user", new String[] { "id", id, "authority", authority.name(), "protocol",
						authority.protocol(), "domain", domain, "user", user });
			} else {
				w.push("user", new String[] { "id", id, "authority", authority.name(), "domain", domain, "user", user });
			}
		} else {
			w.push("user", new String[] { "id", id, "domain", domain, "user", user });
		}
		...
		if (email == null)
			email = ue.stringValue("asset/meta/mf-user/email");
		if (email != null)
	...
	private void describeRoleMember(ServiceExecutor executor, XmlWriter w, String roleMember, boolean projects)
			throws Throwable {
		...
		// "id" and "member" consistent with om.pssd.object.describe usage on a
		// Project object
		if (r != null) {
