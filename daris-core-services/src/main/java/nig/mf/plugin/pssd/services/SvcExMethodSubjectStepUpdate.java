...
public class SvcExMethodSubjectStepUpdate extends PluginService {
    ...
    public SvcExMethodSubjectStepUpdate() {
        ...
        Interface.Element me = new Interface.Element("ps-meta", XmlDocType.DEFAULT,
                "Optional metadata for the project Subject - a list of asset documents.", 0, 1);
        me.setIgnoreDescendants(true);
        _defn.add(me);

        me = new Interface.Element("rs-meta", XmlDocType.DEFAULT,
                "Optional metadata for the RSubject - a list of asset documents.", 0, 1);
        me.setIgnoreDescendants(true);
        _defn.add(me);
    ...
    public String description() {
        return "Updates information for a subject-related step in a locally managed ExMethod. It will automatically set a method namespace of the form <ex method>_<step> on the document.";
    }
    ...
    public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w) throws Throwable {
        ...
        // Set distributed citeable ID for the ExMethod. It is local by
        // definition.
     ...
     * @throws Throwable
     */
    protected static void updateSubject(ServiceExecutor executor, DistributedAsset dEID, String stepPath,
        ...
        if (rsMeta != null && rsMeta.nbElements() > 0) {
            ...
            if (rsid != null) {
                ...
                if (dRSID == null) {
                    throw new Exception("Cannot find the RSubject referred to by the Subject with cid "
                            + dSID.getCiteableID());
                } else {
                    if (!dRSID.isLocal()) {
                        throw new Exception("The RSubject " + dRSID.getCiteableID()
                                + " specified by the parent Subject for this ExMethod is not "
                                + "managed by the local server and cannot be modified");
                    }
                    if (dRSID.isReplica()) {
                        throw new Exception("The RSubject " + dRSID.getCiteableID()
                                + " specified by the parent Subject for this ExMethod is a replica "
                                + " and cannot be modified by this service");
                    }
                }
     ...
     * @param executor
     * @param dID
     *            The distributed asset to update
     * @param mns
     ...
     * @throws Throwable
     */
    private static void setAssetMeta(ServiceExecutor executor, DistributedAsset dID, String mns, XmlDoc.Element meta)
            throws Throwable {
        // Replace/add all of the namespace attributes on the document
        // elements with the method namespace.
        List<XmlDoc.Element> mes = meta.elements();
        for (XmlDoc.Element me : mes) {
            XmlDoc.Attribute xa = me.attribute("ns");

            // Decided not to handle document 'group' attribute for Subject
            // Action steps (see also SvcExMethodStepDescribe)
            // String group = me.value("@group");
            String group = null;
            String ns = ExMethod.addGroupToMetaNamespace(mns, group);
            if (xa == null) {
                me.add(new XmlDoc.Attribute("ns", ns));
            } else {
                xa.setValue(ns); // This means you can't have some other
                                 // namespace...
            }
        }
