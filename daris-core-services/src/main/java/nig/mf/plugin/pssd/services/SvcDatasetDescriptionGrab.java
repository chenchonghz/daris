...
public class SvcDatasetDescriptionGrab extends PluginService {
	private Interface _defn;

	public SvcDatasetDescriptionGrab() {
		...
		_defn.add(new Element("overwrite", BooleanType.DEFAULT,
						"Set to true to overwrite the existing daris:pssd-object/description. Defaults to false.",0,1));

	}

	public String name() {
		return "om.pssd.dataset.description.grab";
	}

	public String description() {
		return "This service extracts the mf-dicom-series/description meta-data element and inserts it into the daris:pssd-object/description field of the given local PSSD DataSet (DICOM) object. If mf-dicom-series/description is not populated, it is generated it from the DICOM file header (using first the series description (0008,103E) and if that is absent the protocol (0018,1030)";
	...
	public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w)
		...
		String pssdDescription = r1.value("asset/meta/daris:pssd-object/description");
		if(pssdDescription!=null&&overwrite==false){
			throw new Exception("There is an existing daris:pssd-object/description element. Set overwrite to true if desired.");
		}
	...
	void updatePssdDescription(String id, String newPssdDescription) throws Throwable {
		if (newPssdDescription==null) return;
		XmlDocMaker doc = new XmlDocMaker("args");
		doc.add("id", id);
		doc.push("meta");
