 ...
 * directly supplied 4) The daris:pssd-notifications meta-data, if set on a
 * Project can also be used to create the list of users to email.
 * 
...
public class MailHandler {
     ...
     *            mail message body
     */
    public MailHandler(String cid, String subject, String message)
            throws Throwable {
     ...
     */
    public void addProjectRole(String projectRole, boolean isExclusive)
            throws Throwable {
        ...
        boolean deRef = true;
        Collection<XmlDoc.Element> members = Project.membersWithProjectRole(
                _thread, proute, _cid, projectRole, isExclusive, memberType,
                deRef, showDetail);
     ...
     * @throws Throwable
     */
    public void addUserMember(String authority, String protocol, String domain,
    ...
    public void addRoleMember(String roleMember) throws Throwable {
        ...
        Collection<XmlDoc.Element> users = Project
                .dereferenceUsersFromRole(_thread, roleMember, null);
        if (users != null)
            _users.addAll(users);
    }
     ...
     * daris:pssd-notification meta-data if it has been set on the Project
     * 
     * @param forDataUpload
     *            If true, looks at the 'data-upload' part of
     *            daris:pssd-notification. If false, nothing is currently
     *            implemented. Probably need to turn this into an enum.
     * @throws Throwable
     */
    public void addNotificationUsers(String category) throws Throwable {

        XmlDocMaker dm = new XmlDocMaker("args");
        dm.add("id", _cid);
        XmlDoc.Element r = _thread.execute("om.pssd.object.describe",
                dm.root());
        XmlDoc.Element notif = r.element("object/meta/daris:pssd-notification");
        if (notif == null) {
            return;
        }
        if (category.equals("data-upload")) {
            addStandardNotificationUsers(notif.element("data-upload"));
        } else if (category.equals("dicom-upload")) {
            addStandardNotificationUsers(notif.element("dicom-upload"));
        }
    }

    private void addStandardNotificationUsers(XmlDoc.Element du)
            throws Throwable {
        if (du == null)
            return;
        ...
        if (roles != null) {
            for (XmlDoc.Element role : roles) {
                if (role != null) {
                    addProjectRole(role.value(),
                            role.booleanValue("@explicit"));
                }
            }
        }
        ...
        if (members != null) {
            for (XmlDoc.Element userMember : members) {
                addUserMember(userMember.value("authority"),
                        userMember.value("protocol"),
    ...
    public void sendMessage(boolean async) throws Throwable {
        ...
        if (_users != null) {
            for (XmlDoc.Element el : _users) {
                ...
                //
                String email = getUserEMail(_thread, authority, protocol,
                        domain, user);
                if (email != null) {
     ...
     * @throws Throwable
     */
    public static void sendAdminMessage(ServiceExecutor executor,
            String projectId, String subject, String msg) throws Throwable {

        XmlDocMaker dm = new XmlDocMaker("args");
        dm.add("id", projectId);
        dm.add("subject", subject);
        dm.add("message", msg);
        dm.add("project-role", new String[] { "explicit", "false" },
    ...
    private String getUserEMail(ServiceExecutor executor, String authority,
            String protocol, String domain, String user) throws Throwable {
     ...
     */
    /*
     * public static void sendMessage(ServiceExecutor executor, String to,
     * String subject, String body) throws Throwable {
     * 
     * XmlDocMaker dm = new XmlDocMaker("args"); dm.add("subject", subject);
     * dm.add("body", body); dm.add("async", true); dm.add("to", to); try {
     * executor.execute("mail.send", dm.root()); } catch (Throwable t) { // Just
     * drop. Don't want exceptions just because we can't send mail
     * System.out.println("Exception caught sending message " + body); } }
     */

    public static void sendMessage(ServiceExecutor executor, String email,
            String subject, String msg, boolean async) throws Throwable {
        ...
        try {
            ...
            System.out.println("Exception caught sending message " + msg);
