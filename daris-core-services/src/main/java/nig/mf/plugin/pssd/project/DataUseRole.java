...
public class DataUseRole {
	...
	public DataUseRole(DataUse dataUse, String cid) {
	...
	public static DataUseRole parse(String role) throws Throwable {

		if (role.startsWith(SPECIFIC_ROLE_PREFIX)) {
			return new DataUseRole(DataUse.specific, role.substring(SPECIFIC_ROLE_PREFIX.length() + 1));
		} else if (role.startsWith(EXTENDED_ROLE_PREFIX)) {
			return new DataUseRole(DataUse.extended, role.substring(EXTENDED_ROLE_PREFIX.length() + 1));
		} else if (role.startsWith(UNSPECIFIED_ROLE_PREFIX)) {
			return new DataUseRole(DataUse.unspecified, role.substring(UNSPECIFIED_ROLE_PREFIX.length() + 1));
		}
