...
public class SvcExMethodStepStudyFind extends PluginService {
	...
	public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w) throws Throwable {
		...
		dm.add("xpath", new String[] { "ename", "type" }, "meta/daris:pssd-study/type");
		dm.add("where", "xpath(daris:pssd-study/method) = '" + id + "' and xpath(daris:pssd-study/method/@step) = '" + stepPath
				+ "'");
		...
		if (aes != null) {
			for (XmlDoc.Element ae : aes) {
				w.push("object", new String[] { "type", Study.TYPE.toString() });
				String proute = ae.value("@proute");
				if (proute == null) {
					w.add("id", new String[] { "asset", ae.value("@id") }, ae.value("cid"));
				} else {
					w.add("id", new String[] { "asset", ae.value("@id"), "proute", proute }, ae.value("cid"));
				}
