...
public class SvcProjectMembersList extends PluginService {
    ...
    public SvcProjectMembersList() {
        _defn = new Interface();
        Interface.Element me = new Interface.Element("id", CiteableIdType.DEFAULT, "The citable ID of the Project.", 1,
                1);
        me.add(new Interface.Attribute(
                ...
                0));
        _defn.add(me);
        _defn.add(new Interface.Element("dereference", BooleanType.DEFAULT,
                "Dereference any role members to users. Defaults to false.", 0, 1));
        _defn.add(new Interface.Element("detail", BooleanType.DEFAULT,
                "Show extra detail on user (email & names). Defaults to false.", 0, 1));
        _defn.add(new Interface.Element("ignore-system", BooleanType.DEFAULT,
                "Ignore SYSTEM domain users ?  Defaults to false.", 0, 1));
    }
    ...
    public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w) throws Throwable {
        ...
        // This function will return their actual project roles as well in the
        // XML
        boolean explicit = false;
        Collection<XmlDoc.Element> members = Project.membersWithProjectRole(executor(), proute, id,
                Project.GUEST_ROLE_NAME, explicit, Project.MEMBER_TYPE_ALL, deRef, showDetail);
        if (members == null)
            return;
