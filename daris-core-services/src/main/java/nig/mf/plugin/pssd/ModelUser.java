...
public class ModelUser {
	...
	public static boolean hasRole(ServerRoute route, ServiceExecutor executor, String role) throws Throwable {
		...
		// numbered explicitly by the peer. E.g. If this is called from 101 an executed
		// on peer 1005 (peer 1005 actor.self.have) the authority used on peer 1005
		// will be 101. So it must be explicitly created there
	...
	/**
	 * Returns the email address of the current user. Null if missing
	 * 
	 * @param executor
	 * @return
	 * @throws Throwable
	 */
	public static String email (ServiceExecutor executor) throws Throwable {
		XmlDoc.Element r = executor.execute("user.self.describe");
		String email = null;
		if (r!=null) {
			email = r.value("user/e-mail");
			if (email==null) {
				email = r.value("user/meta/mf-user/email");
			}
		}
		return email;
	}
	 ...
	 * @throws Throwable
	 */
	public static String domainAndName (ServiceExecutor executor) throws Throwable {
		XmlDoc.Element r = executor.execute("user.self.describe");
		if (r!=null) {
			return r.value("user/@domain") + ":" + r.value("user/@user");
		}
		return null;
	}
