...
public class SvcMethodDescribe extends PluginService {
	...
	public SvcMethodDescribe() {
		...
		Interface.Element me = new Interface.Element("id", CiteableIdType.DEFAULT,
				"The identity of the Method. If not given all matching Methods are returned.", 0, 1);
		...
		_defn.add(me);
		// _defn.add(new
		// Interface.Element("id",CiteableIdType.DEFAULT,"The identity of the method.",0,1));
		_defn.add(new Interface.Element("name", StringType.DEFAULT, "The name of the method (if the id not given)", 0,
				1));
		_defn.add(new Interface.Element("expand", BooleanType.DEFAULT,
				"Should the method be fully expanded - including all references. Defaults to false.", 0, 1));
	}
	...
	public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w) throws Throwable {
		...
		if (id != null) {
			...
			String query = "xpath(daris:pssd-object/name)='" + name + "'";
			dm.add("where", query);
		...
		SvcMethodFind.describeMethods(executor(), w, r.elements("asset"), expand);
	}
