...
public class SvcUserRevoke extends PluginService {
	...
	public SvcUserRevoke() throws Throwable {
		_defn = new Interface();
	...
	public String description() {
		return "Revokes a local user's access to all local PSSD projects and PSSD objects. Does not destroy the underlying Mediaflux account (user.destroy or authentication.user.destroy).";
	}
	...
	public static void removeProjectMember(ServiceExecutor executor, XmlDoc.Element authority, String domain, String user, XmlWriter w) throws Throwable {
		...
		// Remove any registry objects
		XmlDocMaker dm = new XmlDocMaker("args");
		dm.add("access", "private");
		executor.execute("om.pssd.dicom.ae.destroy", dm.root());
	}
