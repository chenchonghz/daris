...
public class SvcObjectFind extends PluginService {
	...
	public SvcObjectFind() {
		...
		_defn = new Interface();
		_defn.add(new Interface.Element("type", new EnumType(new String[] { Project.TYPE.toString(),
				Subject.TYPE.toString(), ExMethod.TYPE.toString(), Study.TYPE.toString(), DataSet.TYPE.toString(),
	...
	public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w) throws Throwable {
		...
		if (type == null) {
			query.append("xpath(daris:pssd-object) has value"); // "cid starts with named id '"
			// +
		...
		if (aes != null) {
			...
			for (XmlDoc.Element ae : aes) {
				...
				if (type != null) { // Only include PSSD objects
					...
					// Establish if the current user has the authorization to edit the
					// meta-data for this object (regardless if they *want* to edit it 
					...
					// Establish if user holds a  system admin role or generic PSSD admin role on the host
					// that manages the object we are describing
					boolean userIsAdmin = ModelUser.hasRole(dAsset.getServerRouteObject(), executor, "system-administrator")
							|| ModelUser.hasRole(dAsset.getServerRouteObject(), executor, Role.objectAdminRoleName());
					
					// Further see if the user holds at least Subject admin role for PSSD tree objects
					// on the server managing the parent project
					if (!type.equals(Method.TYPE)) {
						userIsAdmin = userIsAdmin ||				
							ModelUser.hasRole(dProject.getServerRouteObject(), executor,  Project.subjectAdministratorRoleName(dProject.getCiteableID()));
					}
					...
					if (type == Project.TYPE) {
						...
						addObject = addPssdSubject(executor, dProject.getServerRouteObject(), dAsset, w, ae, forEdit, showSubjectPrivate, userIsAdmin);
	 ...
	 * if (dProjectAsset == null) { throw new
	 * Exception("Cannot find a parent Project of the given object"); }
	 * ServerRoute projectRoute = dProjectAsset.getServerRouteObject(); String
	 * projectCID = dProjectAsset.getCiteableID();
	 * 
	...
	public static void addPssdObject(XmlWriter w, XmlDoc.Element ae, Boolean userIsAdmin) throws Throwable {
		...
		// daris:pssd-filename.  May hold patient details so can be protected as 'private'
		// which is equivalent to our use of namespaces 'pssd.private' in domain-specific meta
		//All PSSD tree objects may have this element, but we have only actually implemented
		// for DataSets
		...
		if (tes != null) {
			for (XmlDoc.Element te : tes) {
				...
				if (ObjectUtil.equals(te.value("name/@dictionary"), ProjectSpecificTagDictionary.dictionaryFor(id).name())) {
	 ...
	 * @throws Throwable
	 */
	public static void addPssdProject(ServiceExecutor executor, XmlWriter w, String projectId, DistributedAsset dAsset,
	 ...
	 *            object regardless of the user's role
	 * @param userUIsAdmin - user is system admin or project admin (project or subject admin)
	 ...
	 * @throws Throwable
	 */
	public static boolean addPssdSubject(ServiceExecutor executor, ServerRoute projectRoute, DistributedAsset dSubject, XmlWriter w,
			XmlDoc.Element ae, boolean forEdit, boolean showSubjectPrivate, boolean userIsAdmin) throws Throwable {
	...
	public static void addPssdExMethod(ServiceExecutor executor, XmlWriter w, XmlDoc.Element ae, boolean forEdit)
		...
		/*
		 * String method = ae.value("meta/daris:pssd-ex-method/method"); if ( method
	...
	public static void addPssdProject(ServiceExecutor executor, String proute, XmlWriter w, XmlDoc.Element ae,
		 ...
		 * we have dropped the daris:pssd-project/member meta-data, we should probably
		 * drop the presentation of member meta-data (done via roles)
		 */
