...
public class SvcProjectCreate extends PluginService {
    ...
    public static void addInterfaceDefn(Interface defn) {
        defn.add(new Interface.Element("namespace", StringType.DEFAULT,
                "The namespace in which to create this project (must pre-exist). Defaults to 'pssd'.", 0, 1));
        defn.add(new Interface.Element("name", StringType.DEFAULT, "The name of this project.", 0, 1));
        defn.add(new Interface.Element("description", StringType.DEFAULT, "An arbitrary description for the project.",
                0, 1));
        ...
        Interface.Element me = new Interface.Element("method", XmlDocType.DEFAULT,
                "Method utilized by this project.  In a federation must be managed by the local server.", 0,
                Integer.MAX_VALUE);
        ...
        me = new Interface.Element("role-member", XmlDocType.DEFAULT,
                ...
                Integer.MAX_VALUE);
        me.add(new Interface.Element("member", StringType.DEFAULT, "The role to become a member of the Project.", 0, 1));
        me.add(new Interface.Element("role", new EnumType(new String[] { Project.ADMINISTRATOR_ROLE_NAME,
                Project.SUBJECT_ADMINISTRATOR_ROLE_NAME, Project.MEMBER_ROLE_NAME, Project.GUEST_ROLE_NAME }),
    ...
    public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w) throws Throwable {
        ...
        String projectRoot = nig.mf.pssd.plugin.util.CiteableIdUtil.citeableIDRoot(executor(), proute, cidRootName);

        // Creator must have project creation role..
        ModelUser.checkHasRole(proute == null ? null : new ServerRoute(proute), executor(),
        ...
        try {
            String cid = nig.mf.pssd.plugin.util.CiteableIdUtil.generateCiteableID(executor(), projectRoot, pdist,
            ...
            if (members != null) {
                for (XmlDoc.Element me : members) {
                    ...
                    if (!r.booleanValue("exists")) {
                        throw new Exception("Cannot add project member: the domain (" + domain + ") and/or user ("
                                + user + ") does not exist.");
                    }
                }
            }
        ...
        // Now check the role members
        {
            ...
            if (members != null) {
                for (XmlDoc.Element me : members) {
                    ...
                    if (!r.booleanValue("exists")) {
                        throw new Exception("Cannot add project member-role: the member-role (" + member
                                + ") does not exist.");
                    }
                }
            }
    ...
    public static String createProjectAsset(ServiceExecutor executor, XmlDoc.Element args, String cid) throws Throwable {
        String selfMemberRole = null;
        ...
        String ns = args.stringValue("namespace", Application.NAMESPACE);

        XmlDocMaker dm = new XmlDocMaker("args");
        dm.add("namespace", ns);
        if (!executor.execute("asset.namespace.exists", dm.root()).booleanValue("exists")) {
            throw new Exception(
                    "The given namespace '"
                            + ns
        ...
        // "daris:pssd-object")
        PSSDUtils.setObjectMeta(dm, Project.TYPE, args.value("name"), args.value("description"), true);
        ...
        if (methods != null || members != null || roleMembers != null) {
            ...
            if (methods != null) {
                for (XmlDoc.Element me : methods) {
                    ...
                    if (!DistributedAssetUtil.assetExists(executor, proute, pdist, id, ResultAssetType.primary, false,
                            pssdOnly, null)) {
            ...
            if (members != null) {
                ...
                for (XmlDoc.Element me : members) {
                    UserCredential userCred = new UserCredential(me.value("authority"),
                            me.value("authority/@protocol"), me.value("domain"), me.value("user"));
                    ...
                    Project.grantProjectRole(executor, cid, userCred, me.value("role"), false); // We
                                                                                                // want
                                                                                                // an
                                                                                                // error
                                                                                                // if
            ...
            if (roleMembers != null) {
                for (XmlDoc.Element me : roleMembers) {
                    ...
                    Project.grantProjectRole(executor, cid, me.value("member"), me.value("role"), false);

                    // Grant data-use role to non p-admin role-members
                    Project.grantProjectRole(executor, cid, me.value("member"), me.value("data-use"), false);
                }
            }
     ...
     * @throws Throwable
     */
    static UserCredential makeSelfCredential(ServiceExecutor executor) throws Throwable {
