...
public class SvcDatasetNameGrab extends PluginService {
	private Interface _defn;

	public SvcDatasetNameGrab() {
		...
		_defn.add(new Element("bruker-fudge", BooleanType.DEFAULT,
				"Fudge: Get the Bruker protocol from the local DataSet name and set daris:bruker-series/protocol with it. Then set the name with the id and protocol. Defaults to false", 0, 1));


	}

	public String name() {
		return "om.pssd.dataset.name.grab";
	}

	public String description() {
		return "This service extracts mf-dicom-series/description(0008,103E)and mf-dicom-series/protocol(0018,1030) out of the DICOM meta-data and insert into daris:pssd-object/name field of the PSSD(dicom) dataset assets. It will try to use {mf-dicom-series/protocol}_{mf-dicom-series/description} first. If they are not available it will then re-populate them from the dicom content header.  For Bruker DataSets, it sets the name from the bit-shifted id and the protocol.";
	}
	...
	public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w)
		...
		if (r1.element("asset/meta/mf-dicom-series") != null) {
			isDicom = true;
		} else if (r1.element("asset/meta/daris:bruker-series") != null) {       // DOcTYpe defined in PSSD package (despite name)
			isDicom = false;
		} else {
			throw new Exception(
					"Asset(id=" + id
		...
		if (isDicom) {
			...
			// Bruker
			String mfdsID = r1.value("asset/meta/daris:bruker-series/id");
			String mfdsProtocol = null;
			
			// The original algorithm set the name=protocol. However it did
			// not put the protocol in daris:bruker-series.  So here we recover
			// the protocol from the name and update the meta-data 
			if (brukerFudge) {
				mfdsProtocol = pssdName;
				XmlDoc.Element brukerMeta = r1.element("asset/meta/daris:bruker-series");
				setBrukerProtocol (id, brukerMeta, mfdsProtocol);
			} else {
				mfdsProtocol = r1.value("asset/meta/daris:bruker-series/protocol");
			}
