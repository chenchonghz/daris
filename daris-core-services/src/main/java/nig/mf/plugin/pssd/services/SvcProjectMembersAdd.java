...
public class SvcProjectMembersAdd extends PluginService {
    ...
    public SvcProjectMembersAdd() {
        ...
        me = new Interface.Element(
                "role-member",
                XmlDocType.DEFAULT,
                "Role to become a member of this project. Potential role members must be a member of the role-member registry; see om.pssd.role-member-registry.list",
                0, 1);
    ...
    public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w) throws Throwable {
        ...
        if (!(ModelUser.checkHasRoleNoThrow(null, executor(), Project.projectAdministratorRoleName(cid)) || ModelUser
                .checkHasRoleNoThrow(null, executor(), Role.objectAdminRoleName()))) {
            throw new Exception(
    ...
    private void add(ServiceExecutor executor, String projectDataUse, Collection<XmlDoc.Element> members,
        ...
        // Find how may project admins there are in the Project. We must leave
        // one at least behind
        ...
        if (members != null) {
            for (XmlDoc.Element member : members) {
                ...
                UserCredential userCred = new UserCredential(member.value("authority"),
                        member.value("authority/@protocol"), member.value("domain"), member.value("user"));
                ...
                // If the user already is a team member they should be removed
                // before being re-added. Leave this to the caller.
                if (currentProjectRole != null) {
                    throw new Exception("The user member " + userCred.toString()
                            + " already exists. You must remove them first with service om.pssd.project.members.remove");
                }
        ...
        if (roleMembers != null) {
            for (XmlDoc.Element member : roleMembers) {
                ...
                // Make sure the specified role is in the role-member registry.
                // Silently drop if not
                // as we don't want to throw an exception in the middle of
                // replacing the whole team
                Boolean checkTopName = false; // The element name is 'member'
                                              // but in the registry it's 'role'
                if (AssetRegistry.checkExists(executor, SvcRoleMemberRegAdd.REGISTRY_ASSET_NAME,
                        SvcRoleMemberRegAdd.DOCTYPE, roleMemberEl, AssetRegistry.AccessType.PUBLIC, checkTopName)) {

                    // Find directly held project role
                    String currentProjectRole = pM.directProjectRole(executor, route, cid);

                    // If the role-member already is a team member they should
                    // be removed before being re-added. Leave this to the
                    // caller.
                    if (currentProjectRole != null) {
                        throw new Exception(
                                "The role member  "
