...
public class SvcProjectMetaDataHarvest extends PluginService {
	...
	public SvcProjectMetaDataHarvest() {
		...
		_defn.add(new Interface.Element("originating-source", UrlType.DEFAULT, "The originating source for any meta-data harvested from this repository; over-rides element value supplied by om.pssd.repository.describe", 0, 1));
		_defn.add(new Interface.Element("over-ride",BooleanType.DEFAULT,"Harvest the meta-data for this project regarldess of the directive in daris:pssd-project-harvest. Only for use in testing; no operational process should set this.  Defaults to false.",0,1));
	}
	...
	public void execute(XmlDoc.Element args, Inputs inputs, Outputs outputs, XmlWriter w) throws Throwable {
		...
		fetchMetaData (executor(), projId, domainService, origSrc, overRide, docOut);
		...
		if (outputs!=null) {
			PluginService.Output o = outputs.output(0);
			o.setData(PluginService.deleteOnCloseInputStream(f), f.length(), "text/xml");				
		}
	...
	private void fetchMetaData (ServiceExecutor executor, String projId, String domainService, 
			XmlDoc.Element origSrc, Boolean overRide, XmlDocMaker docOut) throws Throwable {
		...
		if (r!=null) {
			XmlDoc.Element repos = r.element("repository");
			if (origSrc!=null) {
				XmlDoc.Element origSrcOld = repos.element("originating-source");
				if (origSrcOld==null) {
					repos.add(origSrc);
				} else {
					// This assignment (or setting the value) should update 'repos' by 
					// reference but it doesn't.  So remove and add
					//					origSrcOld = origSrc;                
					repos.removeInstance(origSrcOld);
					repos.add(origSrc);
				}
			}
			//
	...
	private InputStream getXSLT (ServiceExecutor executor) throws Throwable {
		...
		XmlDoc.Element r = executor.execute("asset.meta.transform.profile.describe", dm.root());
	...
	// package
	private XmlDocMaker harvest (ServiceExecutor executor, Boolean overRide, String id) throws Throwable {
		...
		XmlDoc.Element meta = executor.execute("om.pssd.object.describe", dm.root());
		if (meta != null) {

			// See if project has the authorisation meta-data. If not present, opt out
			XmlDoc.Element auth = meta.element("object/meta/daris:pssd-project-harvest");

			// Get some other documents
			XmlDoc.Element pubs = meta.element("object/meta/daris:pssd-publications");
			XmlDoc.Element services = meta.element("object/meta/daris:pssd-related-services");
			XmlDoc.Element governance = meta.element("object/meta/daris:pssd-project-governance");
			XmlDoc.Element category = meta.element ("object/meta/daris:pssd-project-research-category");
			//
			if (auth != null || overRide) {
				...
				if (overRide) {
					...
					allowInstitutional = auth.booleanValue("allow-institutional");
				...
				if (allowInstitutional || overRide) {
					...
					// Fish out the project owner if exists. Could be in two places.
					Collection<XmlDoc.Element> projectOwners = meta.elements("object/meta/daris:pssd-project-owner/project-owner");
					if (projectOwners==null) {
						if (auth!=null) projectOwners = auth.elements("project-owner");
					}
					if (projectOwners != null) {
					...
					// FOR code may be in daris:pssd-project-harvest or daris:pssd-project-research-category
					// wqhich holds generic information about the field of research
					...
					if (category!=null) {
						...
						addFORs(docOut, FORs);
						//
						Collection<String> keywords = category.values("keyword");
						if (keywords!=null) {
					...
					if (governance!=null) {
						Collection<XmlDoc.Element> funding = governance.elements("funding-id");
						if (funding != null) {
						...
						//
						Collection<XmlDoc.Element> ethics = governance.elements("ethics-id"); 
						if (ethics != null) {
							// Dereference the type from nig.funding.organization
	...
	private void addFORs (XmlDocMaker docOut, Collection<String> FORs) throws Throwable {
		if (FORs==null) return;
		for (String FOR : FORs) {
	...
	private void addDocument (XmlDocMaker docOut, String parent, XmlDoc.Element docIn) throws Throwable {
	...
	private void addCounters(ServiceExecutor executor, String id,
		...
		Collection<String> studyTypes = SvcStudyTypesFind.findStudyTypes(executor, id, w);
		if (studyTypes!=null) {
			if (studyTypes.size()>0) {
				meta.push("study-details");
				for (String studyType : studyTypes) {
					int n= nStudies(executor, id, studyType);
					meta.add("type", new String[]{"number", ""+n}, studyType);
				}
				meta.pop();
			}
		}
	...
	private int nStudies(ServiceExecutor executor, String id, String studyType) throws Throwable {
	...
	private int countStudies (ServiceExecutor executor, String id, String studyType) throws Throwable {

		int n = 0;
		XmlDocMaker dm = new XmlDocMaker("args");
		String query = "cid starts with '" + id
				+ "' and xpath(daris:pssd-object/type)='study'";
		if (studyType!=null) query += " and xpath(daris:pssd-study/type)='" + studyType + "'";
		dm.add("where", query);
		dm.add("action", "count");
		dm.add("size", "infinity");
		dm.add("pdist", 0);
	...
	private float sizeOfContent(ServiceExecutor executor, String id, String type)
		...
		XmlDoc.Element r = executor.execute("asset.query", dm.root());
		if (r != null) {
			if (r.element("value").hasValue()) s = r.floatValue("value");
		}
		return s;
	}
