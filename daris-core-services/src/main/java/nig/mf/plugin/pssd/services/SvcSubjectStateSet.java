...
/**
 * This service does not appear to be utilised within our system. It has similar
 * functionality to om.pssd.ex-method.subject.step.update which is utilised.
 * 
 * @author Jason
 * 
 */
public class SvcSubjectStateSet extends PluginService {
    ...
    public SvcSubjectStateSet() {
        ...
        _defn.add(new Interface.Element("ns", StringType.DEFAULT,
                "The namespace of the metadata. Must be specified if 'method' is not specified.", 0, 1));

        Interface.Element me = new Interface.Element("method", CiteableIdType.DEFAULT,
                "The identity of the ExMethod that is updating the state.", 0, 1);
        me.add(new Interface.Attribute("step", CiteableIdType.DEFAULT,
                "The step within the method that resulted in this metadata.", 1));
        _defn.add(me);

        /*
         * Interface.Element we = new
         * Interface.Element("workflow",XmlDocType.DEFAULT
         * ,"Workflow information.",0,1); we.add(new
         * Interface.Element("status",new EnumType(new String[] { "incomplete",
         * "waiting", "complete", "abandoned"
         * }),"The status of the workflow",1,1)); we.add(new
         * Interface.Element("notes"
         * ,StringType.DEFAULT,"Arbitrary workflow notes.",0,1)); _defn.add(we);
         */
