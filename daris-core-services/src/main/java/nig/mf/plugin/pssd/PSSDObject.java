...
public class PSSDObject extends Asset {
	...
	public static enum Type {
		project, subject, ex_method, study, dataset, data_object, r_subject, method;
		...
		public static Type fromId(String cid) {
			if (cid != null) {
		...
		public static Type fromParentId(String pid) {
			if (pid == null || "0".equals(pid)) {
				return project;
			}
			if (CiteableIdUtil.isProjectId(pid)) {
				return subject;
			}
			if (CiteableIdUtil.isSubjectId(pid)) {
				return ex_method;
			}
			if (CiteableIdUtil.isExMethodId(pid)) {
				return study;
			}
			if (CiteableIdUtil.isStudyId(pid)) {
				return dataset;
			}
			if (CiteableIdUtil.isDataSetId(pid)) {
				return data_object;
			}
			return null;
		}
	...
	public PSSDObject(Type type, String name, String description) {
	...
	protected void parseAssetMeta(XmlDoc.Element ae) throws Throwable {
		...
		if (poe != null) {
			_type = Type.parse(poe.value("type"));
	 ...
	 * @throws Throwable
	 */
	public static String move(ServiceExecutor executor, String id, String pid,
		...
		// Set the distation to local. We only want to do this kind of thing on
		// the local server.
		String pdist = null;
		...
		PSSDObject.Type objectType = PSSDObject.Type.parse(r.value("type"));
		if (!(objectType == Subject.TYPE || objectType == ExMethod.TYPE
				|| objectType == Study.TYPE || objectType == DataSet.TYPE || objectType == DataObject.TYPE)) {
			throw new Exception(
					"The object to move must be one of Subject, ExMethod, Study, DataSet or DataObject.");
		}

		boolean includeChildren = false;
		if (!DistributedAssetUtil.assetExists(executor, proute, pdist,
				parentCidOut, DistributedQuery.ResultAssetType.primary,
				includeChildren, true, null)) {
			throw new Exception(
					"The destination parent CID does not have an asset associated with it");
		}
		...
		PSSDObject.Type typeParentOut = PSSDObject.Type.parse(r.value("type"));
		if (!(typeParentOut == Project.TYPE || typeParentOut == Subject.TYPE
				|| typeParentOut == ExMethod.TYPE
				|| typeParentOut == Study.TYPE || typeParentOut == DataSet.TYPE)) {
			throw new Exception(
		...
		if (depthOut + 1 != depthIn) {
			throw new Exception(
					"The destination object CID must have depth one greater than input object");
		}
		...
		if (preserve) {
			...
			objectCidOut = nig.mf.pssd.plugin.util.CiteableIdUtil
					.getFirstAvailableCid(executor, parentCidOut, startId);
		} else {

			// Just get the next CID under the parent
			objectCidOut = nig.mf.pssd.plugin.util.CiteableIdUtil.createCid(
					executor, parentCidOut, pdist);
		}

		// Parent project CIDs
		String projectCidIn = nig.mf.pssd.CiteableIdUtil
				.getProjectId(objectCidIn);
		String projectCidOut = nig.mf.pssd.CiteableIdUtil
				.getProjectId(parentCidOut);
		//
		...
		XmlDoc.Element r2 = executor.execute("om.pssd.collection.members",
				dm.root());
		Collection<String> descendants = r2.values("object/id");

		// Now set the CID on the object from the old to the new tree
		String objectAssetIdIn = nig.mf.pssd.plugin.util.CiteableIdUtil
				.cidToId(executor, objectCidIn);
		dm = new XmlDocMaker("args");
		...
		if (objectType == Study.TYPE) {
			...
			dm = new XmlDocMaker("args");
			dm.add("id", parentCidOut);
			XmlDoc.Element studyMeta = executor.execute(
			...
			if (studyMethod != null) {
				DataSet.updateExMethodMeta(executor, objectCidOut,
						studyMethod.value("id"), studyMethod.value("step"));
			}
		}
	...
	public static boolean isLeaf(ServiceExecutor executor, String id)
			throws Throwable {
	...
	public static String type(ServiceExecutor executor, String id)
			throws Throwable {
		...
		dm.add("xpath", new String[] { "ename", "type" },
				"meta/daris:pssd-object/type");
		XmlDoc.Element r = Asset.query(executor, dm.root());
		return r.value("asset/type");
	}

	public static PSSDObject get(ServiceExecutor executor, ServerRoute sroute,
	...
	public static PSSDObject find(ServiceExecutor executor, String proute,
			String id) throws Throwable {
	...
	public static boolean exists(ServiceExecutor executor, String proute,
			String id) throws Throwable {

		return existsByCid(executor, proute == null ? null : new ServerRoute(
				proute), id);
	}

	public static int countDicomDataSets(ServiceExecutor executor,
			String proute, String id) throws Throwable {
		...
		if (id != null) {
			...
			dm.add("where",
					"( model = 'om.pssd.dataset' ) and ( mf-dicom-series has value )");
		}
		XmlDoc.Element r = executor.execute("asset.query", dm.root());
		return r.intValue("value", 0);
	}

	public static boolean hasSessionLock(ServiceExecutor executor,
	...
	public static boolean hasLock(ServiceExecutor executor, ServerRoute sroute,
			String id) throws Throwable {
	...
	public static int countDataSets(ServiceExecutor executor, String proute,
			String id) throws Throwable {
		...
		if (id != null) {
			...
			dm.add("where",
					"( model = 'om.pssd.dataset' ) and ( asset has content )");
		}
		XmlDoc.Element r = executor.execute("asset.query", dm.root());
		return r.intValue("value", 0);
	}

	public static PSSDObject.Type typeOf(String cid) throws Throwable {
		ServiceExecutor executor = PluginThread.serviceExecutor();
		String type = executor.execute("asset.get",
				"<args><cid>" + cid + "</cid></args>", null, null).value(
				"asset/meta/daris:pssd-object/type");
		if (type == null) {
			throw new Exception(
					"asset "
							+ cid
							+ " is not a valid PSSD object. asset/meta/daris:pssd-object/type is not set.");
		}
		return PSSDObject.Type.parse(type);
	}
