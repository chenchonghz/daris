...
public class SvcUserRoleGrant extends PluginService {
	...
	public static void addInterface(Interface defn) throws Throwable {
		...
		defn.add(new Interface.Element("pssd-role",new EnumType(new String[] { Role.MODEL_USER_ROLE_NAME, Role.POWER_MODEL_USER_ROLE_NAME,
				Role.PROJECT_CREATOR_ROLE_NAME, Role.SUBJECT_CREATOR_ROLE_NAME, Role.OBJECT_ADMIN_ROLE_NAME,
				nig.mf.plugin.pssd.dicom.Role.DICOM_INGEST}), 
				"The generic PSSD role to grant.", 0, Integer.MAX_VALUE));		
	}
	...
	private static void grantRevokePSSDRoles (ServiceExecutor executor,  UserCredential userCred, 
		...
		for (String role : pssdRoles) {
			...
			} else if (role.equalsIgnoreCase(Role.POWER_MODEL_USER_ROLE_NAME)) {
				role = Role.powerModelUserRoleName();
			}  else if (role.equalsIgnoreCase(Role.OBJECT_ADMIN_ROLE_NAME)) {
				role = Role.objectAdminRoleName();
			}
			//
			Project.grantRevoke(executor, userCred, role, create, grant);
		}
