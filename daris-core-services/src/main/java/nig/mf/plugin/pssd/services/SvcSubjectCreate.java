...
public class SvcSubjectCreate extends PluginService {
    ...
    public SvcSubjectCreate() {
        ...
        _defn.add(new Interface.Element("virtual", BooleanType.DEFAULT,
                "Set to true to indicate the subject is a virtual subject. Defaults to false.", 0, 1));
    ...
    public static void addInterfaceDefn(Interface defn) {
        ...
        defn.add(new Interface.Element("description", StringType.DEFAULT, "An arbitrary description for the subject.",
                0, 1));
        ...
        Element me = new Element("public", XmlDocType.DEFAULT,
                "Optional project public metadata - a list of asset documents.", 0, 1);
        me.setIgnoreDescendants(true);
        defn.add(me);

        me = new Element("private", XmlDocType.DEFAULT,
                "Optional project private metadata - a list of asset documents.", 0, 1);
        me.setIgnoreDescendants(true);
        defn.add(me);

        defn.add(new Interface.Element("data-use", new EnumType(new String[] { Project.CONSENT_SPECIFIC_ROLE_NAME,
    ...
    public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w) throws Throwable {
        ...
        // the
        // primary manager of the project. Therefore we must check the user's
        // role with
        // that server. A primary project will always be managed by the same
        // server on which
        // that object exists.
        if (!dPID.isLocal()) {
            Boolean isAdmin = (ModelUser.hasRole(dPID.getServerRouteObject(), executor(),
                    Project.subjectAdministratorRoleName(dPID.getCiteableID())) || ModelUser.hasRole(
                    dPID.getServerRouteObject(), executor(), Role.objectAdminRoleName()));
            if (!isAdmin) {
                throw new Exception("User not authorised: requires '"
                        + Project.subjectAdministratorRoleName(dPID.getCiteableID()) + "' or '"
                        + Role.objectAdminRoleName() + " role on server managing Project : "
        ...
        if (!isAdmin) {
            throw new Exception("User not authorised: requires '"
                    + Project.subjectAdministratorRoleName(dPID.getCiteableID()) + "' or '"
                    + Role.objectAdminRoleName() + " role on local server");
        }
        ...
        try {
            ...
            String cid = createOrUpdateSubjectAsset(executor(), args, null, dPID, subjectNumber, fillIn, actionOnUpdate);
            ...
            try {
                ...
                emid = ExMethod.create(executor(), dCID, exMethodNumber, dMID, fillIn);
            } catch (Throwable t) {
            ...
            SystemEventChannel.generate(new PSSDObjectEvent(Action.CREATE, cid, 1));
        } finally {
            if (fillIn) {
                lock_.unlock();
            }
     ...
     * @throws Throwable
     */
    public static String createOrUpdateSubjectAsset(ServiceExecutor executor, XmlDoc.Element args, String id,
            DistributedAsset dPID, long subjectNumber, boolean fillIn, String actionOnUpdate) throws Throwable {
        ...
        if (mid != null) {
            ...
            if (!mType.equals(Method.TYPE)) {
                throw new Exception("Object " + dPID.getCiteableID() + " [type=" + mType + "] is not a " + Method.TYPE);
            }
            //
            ...
            if (!DistributedAssetUtil.assetExists(executor, dPID.getServerRoute(), pdist, mid, ResultAssetType.primary,
                    false, true, null)) {
                throw new Exception("The Method object (" + mid
        ...
        if (rsid != null) {
            if (!DistributedAssetUtil.assetExists(executor, dPID.getServerRoute(), null, rsid, ResultAssetType.primary,
                    false, true, null)) {
                throw new Exception("The RSubject object (" + rsid
        ...
        dm.add("allow-invalid-meta", args.booleanValue("allow-invalid-meta", false));    // Only available for updates
        ...
        if (id == null) {
            ...
            cid = nig.mf.pssd.plugin.util.CiteableIdUtil.generateCiteableID(executor, dPID.getCiteableID(), pdist,
                    subjectNumber, fillIn);
            dm.add("cid", cid);
            //
        ...
        // Set method and r-subject meta-data which matches the daris:pssd-subject
        // Document Type
        ...
        if (mid != null || rsid != null || dataUse != null) {
            ...
            if (mid != null) {
                ...
                if (id != null) {
                    ...
                    // This is a required field for daris:pssd-subject so we must add
                    // it back in
     ...
     * @throws Throwable
     */
    private static TemplateDefinitions subjectTemplate(ServiceExecutor executor, String proute, String mid)
            throws Throwable {
        XmlDocMaker dm = new XmlDocMaker("args");
        dm.add("cid", mid);
        ...
        tds.publicTemplate = r.element("asset/meta/daris:pssd-method-subject/public");
        tds.privateTemplate = r.element("asset/meta/daris:pssd-method-subject/private");
