...
public class SvcMethodSubjectMetadataDescribe extends PluginService {
	...
	public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w) throws Throwable {
		...
		Vector<SubjectMetadataDefinition> sm = Method.subjectMetadataDefinitions(executor(), dID);
		if ( sm != null ) {
	...
	private void describePSubjectMetadata(XmlWriter w, Vector<SubjectMetadataDefinition> sm) throws Throwable {
	...
	private void describeRSubjectMetadata(XmlWriter w, Vector<SubjectMetadataDefinition> sm) throws Throwable {
	...
	private void describe(XmlWriter w,int type, Vector<SubjectMetadataDefinition> sm) throws Throwable {
		Vector<SubjectMetadataDefinition> ms = filter(type,sm);
		if ( ms == null ) {
			return;
		}
	...
	private Vector<SubjectMetadataDefinition> filter(int type, Vector<SubjectMetadataDefinition> sm) {
		Vector<SubjectMetadataDefinition> fm = null;
		
		for ( int i=0; i < sm.size(); i++ ) {
			SubjectMetadataDefinition md = (SubjectMetadataDefinition)sm.get(i);
			if ( md.subjectType() == type ) {
				if ( fm == null ) {
					fm = new Vector<SubjectMetadataDefinition>();
				}
