...
public class Project {
	...
	public static String projectAdministratorRoleName(String cid) {
		...
		return ProjectRole.PROJECT_ADMINISTRATOR_ROLE_PREFIX + "." + cid;
	}

	public static String subjectAdministratorRoleName(String cid) {

		return ProjectRole.SUBJECT_ADMINISTRATOR_ROLE_PREFIX + "." + cid;
	}

	public static String memberRoleName(String cid) {

		return ProjectRole.MEMBER_ROLE_PREFIX + "." + cid;
	}

	public static String guestRoleName(String cid) {

		return ProjectRole.GUEST_ROLE_PREFIX + "." + cid;
	}
	...
	public static String specificUseRoleName(String cid) {
		return ProjectRole.SUBJECT_DATA_USE_SPECIFIC_PREFIX + "." + cid;
	}
	...
	public static String extendedUseRoleName(String cid) {
		...
		return ProjectRole.SUBJECT_DATA_USE_EXTENDED_PREFIX + "." + cid;
	}
	...
	public static String unspecifiedUseRoleName(String cid) {
		...
		return ProjectRole.SUBJECT_DATA_USE_UNSPECIFIED_PREFIX + "." + cid;
	}
	 ...
	 * @throws Throwable
	 */
	public static Collection<XmlDoc.Element> membersWithProjectRole(ServiceExecutor executor, String proute, String cid,
			String memberRole, boolean explicit, int memberType, boolean deReference, Boolean showDetail)
					throws Throwable {
		...
		for (XmlDoc.Element user : users) {
			...
			XmlDocMaker el = new XmlDocMaker("member",
					new String[] { "id", user.value("@id"), "authority", user.value("@authority"), "protocol",
							user.value("@protocol"), "domain", user.value("@domain"), "user", user.value("@user"),
							"role", projectRole });
			res.add(el.root());
		}
		//
	...
	private static void addUserDetail(ServiceExecutor executor, UserCredential cred, XmlDoc.Element el)
		...
		XmlDoc.Element ue = executor.execute("user.describe", dm.root()).element("user");
		if (ue == null) {
			return;
		}
	...
	public static boolean exists(String projectCid) throws Throwable {
		return PluginThread.serviceExecutor().execute("asset.query", "<args><action>count</action><where>cid='"
				+ projectCid + "' and model='om.pssd.project'</where></args>", null, null).intValue("value") > 0;
	}

	public static String projectSpecificDictionaryNamespaceOf(String projectCid) {
		return "daris-project-" + projectCid;
	}

	public static void createProjectSpecificDictionaryNamespace(ServiceExecutor executor, String projectCid)
			throws Throwable {
		String dictionaryNamespace = projectSpecificDictionaryNamespaceOf(projectCid);
		XmlDocMaker dm = new XmlDocMaker("args");
		dm.add("ifexists", "ignore");
		dm.add("namespace", dictionaryNamespace);
		dm.add("description", "Project specific namespace for DaRIS project " + projectCid);
		executor.execute("dictionary.namespace.create", dm.root());
	}

	public static void grantProjectSpecificDictionaryNamespacePermissions(ServiceExecutor executor, String projectCid)
			throws Throwable {
		...
		dm.push("perm");
		dm.add("resource", new String[] { "type", "dictionary:namespace" }, dictionaryNamespace);
	...
	public static void destroyProjectSpecificDictionaryNamespace(ServiceExecutor executor, String projectCid)
			throws Throwable {
		String dictionaryNamespace = projectSpecificDictionaryNamespaceOf(projectCid);

		if (executor
				.execute("dictionary.namespace.exists",
						"<args><namespace>" + dictionaryNamespace + "</namespace></args>", null, null)
