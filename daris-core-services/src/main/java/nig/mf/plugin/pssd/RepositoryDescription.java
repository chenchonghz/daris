...
public class RepositoryDescription {
	...
	private static final String DOC_TYPE = "daris:pssd-repository-description";
	...
	public static String findAndCreateRepositoryDescription(
		...
		if (id == null) {
			...
			// able to manipulate it. End users should have the ability to see it.
			dm.push("acl");
	 ...
	 * @param the
	 *            daris:pssd-repository-description record to add
	 * 
	 * @throws Throwable
	 */
	public static void replaceRecord(ServiceExecutor executor, String id,
			...
			XmlDoc.Element holdings, XmlDoc.Element origSrc) throws Throwable {

		XmlDocMaker dm = new XmlDocMaker("args");
		dm.add("id", id);
		dm.push("meta", new String[] { "action", "replace" });
		dm.push(DOC_TYPE);
		if (name != null) dm.add(name);
		if (custodian != null) dm.add(custodian);
		if (location != null) dm.add(location);
		if (rights != null) dm.add(rights);
		if (holdings != null) dm.add(holdings);
		if (origSrc!=null) dm.add(origSrc);
		//
		dm.pop();
		dm.pop();
	...
	public static XmlDoc.Element getRepositoryDescription(ServiceExecutor executor, Boolean useANDSDateFormat) throws Throwable {
		...
		if (useANDSDateFormat && r!=null) {
			XmlDoc.Element date = r.element("asset/meta/daris:pssd-repository-description/data-holdings/start-date");
			if (date!=null) {
				formatDate (date);
			}
		}
		return r.element("asset/meta/daris:pssd-repository-description");
	}
