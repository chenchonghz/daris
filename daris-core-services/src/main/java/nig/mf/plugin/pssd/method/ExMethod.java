...
public class ExMethod extends PSSDObject {
    public static final PSSDObject.Type TYPE = PSSDObject.Type.ex_method;
    ...
    private List<ExMethodStepStatus> _steps; // Only steps that have status
    ...
    /**
     * All steps for which there is information on the status. If not, the step
     ...
     * @param executor
     * @param dSID
     *            Distributed citeable ID for parent Subject
    ...
    /**
     * Is the specified ExMethod in use by any Study? If the Study is
     ...
     * @throws Throwable
     */
    public static boolean inUseByAnyStudy(ServiceExecutor executor, DistributedAsset dEMID, String pdist)
            throws Throwable {
     ...
     * @throws Throwable
     */
    public static long studyUseCount(ServiceExecutor executor, DistributedAsset dEMID, String pdist, String maxSize)
            throws Throwable {

        // Prepare query
        String emid = dEMID.getCiteableID();

        // Primary projects have primary ExMethods. Replica projects have
        // replica ExMethods.
        // Note that the Study says which ExMEthod it uses, not which Method,
        // despite the naming
        // of the 'method' element.
        String query = null;
        if (dEMID.isReplica()) {
            query = "xpath(daris:pssd-study/method)='" + emid + "' and rid has value";
        } else {
            query = "xpath(daris:pssd-study/method)='" + emid + "' and rid hasno value";
        }
    ...
    /**
     * Creates an asset that contains a copy of the template used to execute
     * this method.
     * 
     * @param executor
     * @param mid
    ...
    public void parseAssetMeta(XmlDoc.Element ae) throws Throwable {
        ...
        XmlDoc.Element me = ae.element("meta/daris:pssd-ex-method");
        if (me == null) {
        ...
        // the constructor that supplies the higher-level info (version/authors
        // etc)
        ...
        me = ae.element("meta/daris:pssd-method");
        if (ae != null) {
    ...
    /**
     * If non null, add group to <ex method>_<step> making <ex
     * method>_<step>_<group>
     * 
     * @param mns
     * @param group
     * @return
     */
    public static String addGroupToMetaNamespace(String mns, String group) {
        if (group != null) {
            return mns + "_" + group;
        } else {
            return mns;
    ...
    /**
     * The namespace for Method steps is of the form <ex method>_<step>_<group>
     * Pull off the group and return the front two parts. If group is not there
     * just returns the input
     * 
     * @param ns
     * @return
     */
    public static String partialMetaNameSpace(String ns) {
