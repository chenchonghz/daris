...
public class SvcExMethodStepUpdate extends PluginService {
    ...
    public static void addInterfaceDefn(Interface defn) {
        defn.add(new Interface.Element("id", CiteableIdType.DEFAULT,
                "The identity of the ExMethod object. Must be managed by the local server.", 0, 1));
        defn.add(new Interface.Element("step", CiteableIdType.DEFAULT, "The step within the method(s).", 1, 1));
        defn.add(new Interface.Element("state", new EnumType(new String[] { ExMethodStepStatus.STATUS_INCOMPLETE,
                ExMethodStepStatus.STATUS_WAITING, ExMethodStepStatus.STATUS_COMPLETE,
                ExMethodStepStatus.STATUS_ABANDONED }), "The current state of execution.", 1, 1));
    ...
    /**
     * Update the Method-related meta-data in a local ExMethod. IT is the
     * callers responsibility to ensure that the object to be updated is
     ...
     * @throws Throwable
     */
    public static void updateExMethod(ServiceExecutor executor, String emid, String stepPath, XmlDoc.Element args)
            throws Throwable {
