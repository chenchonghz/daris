...
public class SvcStudyRename extends PluginService {
    ...
    public SvcStudyRename() throws Throwable {
        ...
        _defn.add(new Interface.Element("id", CiteableIdType.DEFAULT,
                "The citeable ID of the parent object. It can be a Project, a Subject, an ExMethod or a Study.", 1, 1));
        Interface.Element e = new Interface.Element("element", StringType.DEFAULT,
                "The XPATH to the document element/attribute to be included in the new study name.", 1,
                ELEMENT_MAX_OCCURS);
        e.add(new Interface.Attribute(
                "idx",
    ...
    public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w) throws Throwable {
        ...
        PSSDObject.Type type = PSSDObject.Type.parse(nig.mf.pssd.plugin.util.PSSDUtil.typeOf(executor(), rootId));
        if (type == null) {
            throw new Exception("The asset associated with " + rootId.toString() + " does not exist");
        }
        if (!type.equals(Study.TYPE) && !type.equals(ExMethod.TYPE) && !type.equals(Project.TYPE)
                && !type.equals(Subject.TYPE)) {
            throw new Exception("Invalid root citeable id. It has to be a Project, Subject, Study, or an ExMethod.");
        }
        if (rootId.isReplica()) {
            throw new Exception("The supplied object is a replica and this service cannot rename it or its children.");
        }

        // This could easily be relaxed as we are going to rename local
        // children. But
        ...
        if (rootId.hasRemoteChildren(ResultAssetType.primary, ptag, pdist)) {
            throw new Exception(
                    "The supplied object has primary children on remote servers in the federation; cannot rename children.");
        }

        // Find all Studies belonging to the given parent. Can't do this with an
        // om.* service...
        String rootCID = rootId.getCiteableID();
        List<XmlDoc.Element> ses = sortElements(args.elements("element"));
        String queryString = "(cid starts with '" + rootCID + "' or cid = '" + rootCID
    ...
    private Vector<XmlDoc.Element> sortElements(Collection<XmlDoc.Element> es) throws Throwable {
    ...
    private String createName(List<XmlDoc.Element> ses, XmlDoc.Element meta) throws Throwable {
        String name = "";
        for (XmlDoc.Element e : ses) {
