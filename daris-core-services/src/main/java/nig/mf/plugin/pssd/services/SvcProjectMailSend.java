...
public class SvcProjectMailSend extends PluginService {
	...
	public SvcProjectMailSend() {
		...
		me = new Interface.Element("use-notification", BooleanType.DEFAULT,
				"Send email to users according to the daris:pssd-notification meta-data attached to the Project.", 0, 1);
		me.add(new Interface.Attribute("category", new EnumType(new String[] {"data-upload", "dicom-upload"}),
				"Select the category in daris:pssd-notification to select which users are emailed. Default is 'data-upload'.", 0));
		_defn.add(me);	
		//
	...
	private void sendToNotifications (ServiceExecutor executor, MailHandler mh, XmlDoc.Element args) throws Throwable {
		...
		if (useNotification) {
			...
			if (category.equals("data-upload")) {
				mh.addNotificationUsers("data-upload");
			} else if (category.equals("dicom-upload")) {
				mh.addNotificationUsers("dicom-upload");
