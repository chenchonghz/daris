...
public class SvcRepositoryDescribe extends PluginService {
    ...
    public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w) throws Throwable {
        ...
        // Add the asset ID if the repository description asset has been made
		String assetID = RepositoryDescription.findRepositoryDescription(executor());
		if (assetID!=null) w.add("asset-id", assetID);

        w.add("id", new String[] { "method-id-root", methodIdRoot, "project-id-root", projectIdRoot }, uuid);
        w.add("number-of-projects", SvcCollectionMemberCount.countMembers(null));
