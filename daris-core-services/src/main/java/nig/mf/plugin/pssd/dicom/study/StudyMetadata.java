...
public class StudyMetadata {
	...
	/**
	 * Specialised partial restoration of StudyMetadata with just the patient details.
	 * 
	 * @param patientID
	 * @param sex
	 * @param dob
	 ...
	 * @param lastName
	 */	
	public static StudyMetadata createFrom (String patientID, String sex, Date dob, String firstName, String lastName) {
		StudyMetadata sm = new StudyMetadata();
	 ...
	 * Used by the domain-dependent meta-data framework (see DomainMetaData)
	 * 
	 * @return
	 * @throws Throwable
	 */
	public XmlDoc.Element toXML () throws Throwable {
		...
		if (_protocolName!=null) dm.add("protocol", _protocolName);
		...
		if (_model!=null) dm.add("model", _model);
		if (_magneticFieldStrength!=null) dm.add("magnetic_field_strength", _magneticFieldStrength);
		//
		dm.push("subject");
		...
		if (_patientDOB!=null) dm.add("dob", _patientDOB);
		dm.pop();
		//
		return dm.root();
	}
	...
	private void restore(XmlDoc.Element meta) throws Throwable {
		...
		// Elements that are not provided will end up at their init values (null or -1)
		_uid =  meta.value("uid");
		_id = meta.value("id");
		...
		if (subject!=null) {
			...
			_patientDOB = meta.element("dob")!=null ? meta.dateValue("dob") : null;
	 ...
	 * @param lastName
	 */
	private void restore (String patientID, String sex, Date dob, String firstName, String lastName) {
		init();
		_patientID = patientID;
		_patientSex = sex;
		_patientDOB = dob;
		_patientName = new DicomPersonName(null, null, firstName, null, lastName, null);
		_havePatientDetails = true;
	}
	...
	private void restore(DataElementMap dem) throws Throwable {
		...
		_patientDOB = ( patientBirthDate == null ) ? null : DicomDateTime.dateTime(patientBirthDate,patientBirthTime);
		if ( _patientDOB != null ) {
			_havePatientDetails = true;
		}
		...
		_magneticFieldStrength = dem.stringValue(DicomElements.MAGNETIC_FIELD_STRENGTH);
	}
