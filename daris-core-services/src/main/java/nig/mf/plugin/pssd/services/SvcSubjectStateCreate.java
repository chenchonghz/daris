 ...
 * type daris:pssd-state has never been defined/created in our system. So looks like a
 * path JL abandoned.
 * 
 * @author Jason
 * 
...
public class SvcSubjectStateCreate extends PluginService {
    ...
    public SvcSubjectStateCreate() {
        ...
        Interface.Element me = new Interface.Element("method", CiteableIdType.DEFAULT, "The identity of the Method.",
                0, 1);
        me.add(new Interface.Attribute("step", IntegerType.POSITIVE_ONE,
                "The step within the method that resulted in this Study.", 1));
        _defn.add(me);
        //
        Interface.Element we = new Interface.Element("workflow", XmlDocType.DEFAULT, "Workflow information.", 0, 1);
        we.add(new Interface.Element("status", new EnumType(new String[] { "incomplete", "waiting", "complete",
                "abandoned" }), "The status of the workflow", 1, 1));
    ...
    private static void createState(ServiceExecutor executor, String id, XmlDoc.Element args) throws Throwable {
        ...
        XmlDoc.Element ses = r.element("asset/meta/daris:pssd-state");
        if (ses == null) {
            ses = new XmlDoc.Element("daris:pssd-state");
        }
        ...
        if (me != null) {
            // Check the meta is indeed conformant to the prescribed
            // definitions..
