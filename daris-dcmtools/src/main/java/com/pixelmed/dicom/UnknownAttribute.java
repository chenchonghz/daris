...
/**
 * <p>
 * A concrete class specializing {@link com.pixelmed.dicom.Attribute Attribute}
 * for Unknown (UN) attributes.
 * </p>
 *
 * <p>
 * Though an instance of this class may be created using its constructors, there
 * is also a factory class, {@link com.pixelmed.dicom.AttributeFactory
 * AttributeFactory}.
 * </p>
 *
...
public class UnknownAttribute extends Attribute {
    private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/dicom/UnknownAttribute.java,v 1.22 2016/02/04 09:05:27 dclunie Exp $";

    private static final Logger slf4jlogger = LoggerFactory
            .getLogger(UnknownAttribute.class);
     ...
     */
    public UnknownAttribute(AttributeTag t, long vl, DicomInputStream i)
    ...
    private void doCommonConstructorStuff(long vl, DicomInputStream i)
        ...
        try {
            ...
            // throw new DicomException("Failed to skip value (length "+vl+"
            // dec) in UN attribute "+getTag());
            throw new DicomException("Failed to read value (length " + vl
                    + " dec) in UN attribute " + getTag());
        }
    }
     ...
     *         {@link com.pixelmed.dicom.ValueRepresentation
     *         ValueRepresentation}
     */
    public byte[] getVR() {
     ...
     *
     * <p>
     * Always to be interpreted as little endian, per the DICOM definition of
     * UN, regardless of the received transfer syntax.
     * </p>
     *
     ...
     *
     * <p>
     * Assumes the caller knows that the UN VR is really a valid string (e.g.,
     * knows the VR of a private attribute).
     * </p>
     *
     * <p>
    ...
    public String[] getStringValues(NumberFormat format) throws DicomException {
        ...
        // should really check for SpecificCharacterSet rather than using
        // default encoding :(
        String[] originals = { new String(originalLittleEndianByteValues) };
        return ArrayCopyUtilities
                .copyStringArrayRemovingLeadingAndTrailingPadding(originals);
    }
     ...
     *
     * <p>
     * Assumes the caller knows that the UN VR is really a valid FD (e.g., knows
     * the VR of a private attribute).
     * </p>
     *
     * @throws DicomException
     */
    public double[] getDoubleValues() throws DicomException {
        ...
        if (valueLength % FloatDoubleAttribute.bytesPerValue != 0) {
            throw new DicomException("incorrect value length (" + valueLength
                    + " dec) for UN as FD");
        } else {
            try {
                BinaryInputStream ibs = new BinaryInputStream(
                        new ByteArrayInputStream(
                                originalLittleEndianByteValues),
                        false/* big endian */);
                int vm = (int) (valueLength
                        / FloatDoubleAttribute.bytesPerValue);
     ...
     *
     * <p>
     * Assumes the caller knows that the UN VR is really a valid FL (e.g., knows
     * the VR of a private attribute).
     * </p>
     *
     * @throws DicomException
        ...
        if (valueLength % FloatSingleAttribute.bytesPerValue != 0) {
            ...
            try {
                BinaryInputStream ibs = new BinaryInputStream(
                        ...
                        false/* big endian */);
                int vm = (int) (valueLength
                        / FloatSingleAttribute.bytesPerValue);
                values = new float[vm];
                ;
                for (int j = 0; j < vm; ++j) {
