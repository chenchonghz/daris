...
public class PrimaryDataSet extends DataSet {
	...
	public static PrimaryDataSet create(ServerClient.Connection cxn, boolean isImage, String pid, String name, String description,
			String seriesUID, String seriesID, String protocol, String acqTime, File f, String contentMimeType) throws Throwable {

		ServerClient.Input in = ServerClient.createInputFromURL("file:" + f.getAbsolutePath());
		if (contentMimeType!=null) in.setType(contentMimeType);
		String id;
		try {
			...
			w.push("meta");
			w.push("daris:bruker-series", new String[] { "ns", PSSDUtil.BRUKER_META_NAMESPACE });
			if (seriesUID != null) {
				w.add("uid", seriesUID);
			}
			if (seriesID != null) {
	...
	public static PrimaryDataSet find(ServerClient.Connection cxn, String studyCID, String seriesUID, String seriesID)
		...
		if (seriesUID != null) {
			where += " and xpath(daris:bruker-series/uid) = '" + seriesUID + "'";
		}
		...
		String protocol = r.value("asset/meta/daris:bruker-series/protocol");
		String acqTime = r.value("asset/meta/daris:bruker-series/acqTime");

		// Query to check if the found primary dataset has derivation
		w = new XmlStringWriter();
		w.add("where", "model = 'om.pssd.dataset' and xpath(daris:pssd-derivation/input) = '" + id + "'");
	...
	public static boolean hasDicomDerivation(ServerClient.Connection cxn, String primaryId) throws Throwable {
		...
		w.add("where", "type='" + MimeTypes.DICOM_SERIES + "'" + " and model = 'om.pssd.dataset' "
				+ " and xpath(daris:pssd-derivation/input) = '" + primaryId + "'");
		XmlDoc.Element r = cxn.execute("asset.query", w.document());
		if (r.value("id") != null) {
			return true;
		} else {
