...
public class Study extends BaseObject {
	...
	public static Study create(ServerClient.Connection cxn, String pid, String name, String description,
			String studyUID, String studyID, UserCredential cred) throws Throwable {
		...
		w.push("meta");
		w.push("daris:bruker-study ", new String[] { "ns", PSSDUtil.BRUKER_META_NAMESPACE });
		w.add("id", studyID);
		w.add("uid", studyUID);
		//
		w.push("ingest");
	...
	public static Study createFromSubjectCID(ServerClient.Connection cxn, String subjectCID, String name,
			String description, String studyUID, String studyID, UserCredential cred) throws Throwable {
		...
		return create(cxn, exm.id(), name, description, studyUID, studyID, cred);
	}
	...
	public static Study find(ServerClient.Connection cxn, String subjectCID, String exMethodCID, String studyUID) throws Throwable {
		...
		w.add("where", cidQuery + " and model = 'om.pssd.study' and " + "( xpath(mf-dicom-study/uid) = '" + studyUID
				+ "' or xpath(daris:bruker-study /uid) = '" + studyUID + "' )");
		...
		String name = r.value("asset/meta/daris:pssd-object/name");
		String description = r.value("asset/meta/daris:pssd-object/description");
		
		boolean hasDicom =  r.element("asset/meta/mf-dicom-study") != null;
		boolean hasBruker = r.element("asset/meta/daris:bruker-study ") != null;
		...
		if (hasDicom && hasBruker) {
			...
			String uidBruker = r.value("asset/meta/daris:bruker-study /uid");
			if (!uidDicom.equals(uidBruker)) {
				String errMsg = "daris:bruker-study /uid is not equal to mf-dicom-study/uid on PSSD study asset(cid=" + id
				+ ").";
				LogUtil.logError(cxn, PSSDUtil.BRUKER_LOG_FILE, errMsg);
				throw new Exception(errMsg);
			}
		...
		String studyID = r.value("asset/meta/daris:bruker-study /id");
		if (studyID == null) {
