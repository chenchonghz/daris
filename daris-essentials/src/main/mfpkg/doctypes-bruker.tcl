...
proc createDocType_hfi_bruker_study { } {
	...
	asset.doc.type.update \
		...
		> \
		:element -name date -type date -index true -min-occurs 0 -max-occurs 1 < :description "Date of acquisition" > \
		:element -name coil -type string -index true -min-occurs 0 -max-occurs 1 < :description "Scanner coil used for acquisition" > \
...
proc destroyPSSDDocTypesBruker {} {
	...
	set doctypes { daris:bruker-series daris:bruker-study }
