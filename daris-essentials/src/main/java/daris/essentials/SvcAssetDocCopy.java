...
public class SvcAssetDocCopy extends PluginService {
	...
	public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w) throws Throwable {
		...
		List<XmlDoc.Element> docNames = args.elements("doc");
	...
	private void setDocAttributes(List<XmlDoc.Element> docs, String ns, String tag) {
		if (docs == null) {
			return;
		}
		for (XmlDoc.Element doc : docs) {
	...
	private List<XmlDoc.Element> findDocs(XmlDoc.Element asset, List<XmlDoc.Element> docNames) throws Throwable {
		if (asset == null || docNames == null) {
			return null;
		}
		List<XmlDoc.Element> rdocs = new Vector<XmlDoc.Element>();
		for (XmlDoc.Element doc : docNames) {
			String docName = doc.value();
			String ns = doc.value("@ns");
			String tag = doc.value("@tag");
			XmlDoc.Element rdoc = findDoc(asset, docName, ns, tag);
			if (rdoc != null) {
				rdocs.add(rdoc);
			}
		}
