...
public class SvcDICOMSendEss extends PluginService {
    ...
    public SvcDICOMSendEss() throws Throwable {
        ...
        me.add(new Interface.Element("host", StringType.DEFAULT,
                "The AE host name or IP address.", 1, 1));
        me.add(new Interface.Element("port", IntegerType.DEFAULT,
                "The port number of the AE", 1, 1));
        me.add(new Interface.Element("aet", StringType.DEFAULT,
                "The AET of the AE.", 1, 1));
        _defn.add(me);

        //
        _defn.add(new Interface.Element("aet", StringType.DEFAULT,
                "The calling (our) AET.", 1, 1));
        ...
        me = new Interface.Element(
                ...
                "Citable ID to encode in temporary DICOM meta-data before sending.",
                0, 1);
        me.add(new Interface.Attribute(
                "group",
                StringType.DEFAULT,
                "The DICOM group component to overload with the CID. Part of the form <group,element>."));
        me.add(new Interface.Attribute(
                "element",
                StringType.DEFAULT,
                "The DICOM element component to overload with the CID. Part of the form <group,element>"));
        _defn.add(me);
        //
        _defn.add(new Interface.Element(
                "exception-on-fail",
        ...
        _defn.add(new Interface.Element(
                "log",
                BooleanType.DEFAULT,
                "Write an entry in the mediaflux log file as assets are sent (defaults to false).",
                0, 1));
    }
    ...
    public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w)
        ...
        if (patients != null) {
            if (studies != null || series != null) {
                throw new Exception(
                        "You can only give one of patient/study/series");
            }
        } else {
            if (studies != null && series != null) {
                throw new Exception(
        ...
        if (where != null) {
            if (patients != null || studies != null || series != null) {
                throw new Exception(
                        "You can only give one of 'where' and 'patient/study/series'");
            }
        }
        //
        if (patients == null && studies == null && series == null
                && where == null) {
            throw new Exception(
                    "One of 'where' and 'patient/study/series' must be supplied");
        }
        //
        String patientNameAction = args.stringValue("patient-name-action",
                "unchanged");
        boolean anonymise = false;
        if (patientNameAction.equalsIgnoreCase("anonymize"))
            anonymise = true;
        ...
        if (cid != null) {
            ...
            if (group == null || element == null) {
                throw new Exception(
                        "You must give both the group and element to specify the DICOM meta-data element holding the CID.");
            }

            if (anonymise) {
                if (group.equals("0010") && element.equals("0010")) {
                    throw new Exception(
                            "You have asked to anonymize the patient name and overload it with a CID. This is not self consistent.");
                }
            }
        }
        //
        ...
        Collection<String> assets = DICOMModelUtil.findSeries(executor(),
                where, patients, studies, series);
        ...
        if (assets != null) {
            ...
            for (String asset : assets) {
                ...
                //
                PluginTask.setCurrentThreadActivity("Extracting "
                        + (anonymise ? "and anonymizing" : "")
                ...
                try {
                    PluginTask
                            .setCurrentThreadActivity("Sending DICOM dataset "
                                    + asset);
                    new StorageSOPClassSCU(host, port, calledAET, callingAET,
                            dcmFiles, compressionLevel, dh, null, 0, debugLevel);
                    if (dh.getNumberFailed() > 0) {
                        if (exceptionOnFail) {
                            throw new Exception(
                                    "Failed to send DICOM data for cid "
                                            + asset);
                        }
                        w.add("asset",
                                new String[] { "status", "fail", "completed",
                                        ...
                                        "" + dh.getNumberFailed() }, asset);
                        if (writeLog)
                            System.out.println("Asset " + asset
                                    + " not successfully sent");
                    } else {
                        w.add("asset", new String[] { "status", "pass",
                                "completed", "" + dh.getNumberCompleted() },
                                asset);
                        if (writeLog)
                            System.out.println("Asset " + asset
                                    + " successfully sent");
                    ...
                    if (exceptionOnFail) {
                        throw new Exception(
                                "Failed to make association with AE");
                    }

                    w.add("asset", new String[] { "status", "fail", "reason",
                            "association failed" }, asset);
                }
                // Clean up
    ...
    private static void editDICOM(File tempDir, boolean anonymise,
            XmlDoc.Element cid, SetOfDicomFiles dcmFiles) throws Throwable {
        ...
        for (int i = 0; i < files.length; i++) {
            File f = files[i];
            if (anonymise)
                DicomModify.editFile(f, aTag, " ");
            if (bTag != null)
                DicomModify.editFile(f, bTag, cid.value());
