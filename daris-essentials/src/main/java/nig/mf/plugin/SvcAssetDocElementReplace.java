...
public class SvcAssetDocElementReplace extends PluginService {
	...
	public SvcAssetDocElementReplace() {
		...
		_defn.add(new Element("pid", CiteableIdType.DEFAULT,
				"The parent citable ID of the asset of interest (give this or cid or neither but not both).", 0, 1));
		_defn.add(new Element("type", StringType.DEFAULT, "The document type of interest.", 1,
				1));
	...
	public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w) throws Throwable {
		...
		if (cid!=null && pid!=null) {
			throw new Exception ("Can't give 'pid' and 'cid'");
		}
		...
		//
		replace(executor(), cid, pid, assetType, docType, elementPath, attName, oldValues, newValues, w);
	}

	private void replace(ServiceExecutor executor, String cid, String pid, DistributedQuery.ResultAssetType assetType,
		...
		StringBuilder query = new StringBuilder("xpath(" + docType + ") has value");
		...
		if (cid != null) {
			...
			query.append(" and cid starts with '" + pid + "'");
		}
