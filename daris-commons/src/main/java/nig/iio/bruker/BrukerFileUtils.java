...
import nig.compress.TarUtil.CompressionType;
...
/**
 * 
 * This class provides methods to access Bruker ParaVision data directories and
 * files. Below is the directory structure of Bruker ParaVision data directory:
 * 
 * ${disk_unit}/${user}/nmr/${name}/${expno}/pdata/${procno}
 * ${disk_unit}/${user}
 * /nmr/${study_name}/${acquisition_no}/pdata/${reconstruction_no}
 * ${disk_unit}/${user}/nmr/${session_name}/${run_no}/pdata/${reconstruction_no}
 * 
 * Note: Depend on the source of the documentation, the description about the
 * Bruker ParaVision data directory uses different terms: study <=> session
 * exp(experiment) <=> run <=> acq(acquisition) <=> series pdata(processed data)
...
public class BrukerFileUtils {
    ...
    /**
     * Check if the specified directory is a Bruker acquisition/series
     * directory.
     * 
     * @param dir
     * @return
    ...
    /**
     * Check if the specified directory is a Bruker pdata(Processed Data)
     * directory.
     * 
     * @param dir
     * @return
    ...
    /**
     * Check if the specified directory is a Bruker reconstruction directory.
     * Normally an acquisition has only one reconstruction, but it is possible
     * that an acquisition has multiple reconstructions. Therefore, if
     * converting bruker data to other formats, each reconstruction within an
     * acquisition will become an image of other format (e.g. a DICOM series, an
     * Analyze/Nifti image)
     * 
     * @param dir
     * @return
    ...
    /**
     * Get the acquisition sub-directories within the specified study/session
     * directory.
     * 
     * @param studyDir
    ...
    /**
     * Get the reconstruction sub-directories within the specified acquisition
     * or pdata directory.
     * 
     * @param dir
    ...
    /**
     * Create a series container file. It compresses the reco directory and
     * other required meta files in its parent directories.
     * 
     * @param recoDir
     * @param clevel
     *            compression level
     * @throws Throwable
     */
    public static File createImageSeriesContainer(File recoDir, boolean fidWithImage, int clevel, String ctype)
            throws Throwable {

        // Find file
        Set<File> files = new TreeSet<File>();
        findImageFilesToCompress(recoDir, files, fidWithImage);

        // Package into container
        File container = null;
        if (ctype.equals("zip")) {
            container = File.createTempFile("nig_pv_upload", ".zip");
            ZipUtil.zip(files, getParentNmrDir(recoDir), container, clevel);
        } else if (ctype.equals("tar")) {
            container = File.createTempFile("nig_pv_upload", ".tar");
            TarUtil.tar(files, getParentNmrDir(recoDir), container, CompressionType.NONE);
        } else if (ctype.equals("aar")) {
            container = File.createTempFile("nig_pv_upload", ".aar");
            ArchiveUtil.compress(files, getParentNmrDir(recoDir), container, MimeTypes.AAR, clevel, null);
        }
    ...
    /**
     * Find the files to create series archive. All the required files are
     * stored in the specified Set<File>
     * 
     * @param dir
     * @param filesToZip
     */
    private static void findImageFilesToCompress(File dir, Set<File> filesToCompress, boolean fidWithImage) {
        ...
        for (int i = 0; i < files.length; i++) {
            // By only finding non-directories, we exclude other acquisition
            // directories
            if (!files[i].isDirectory()) {

                // Filter out the fid (raw Fourier data). This is uploaded to a
                // separate DataSet
                if (fidWithImage) {
                    filesToCompress.add(files[i]);
                } else {
        ...
        findImageFilesToCompress(dir.getParentFile(), filesToCompress, fidWithImage);
    }

    /**
     * Create a fid container file. It compresses the full tree for this
     * acquisition starting at the Study level but stops at the pdata level
     * 
     * @param acqDir
     * @param clevel
     *            compression level
     * @throws Throwable
     */
    public static File createFidContainer(File acqDir, int clevel, String ctype) throws Throwable {

        // FInd files
        Set<File> files = new TreeSet<File>();
        findFidFilesToCompress(acqDir, files);

        // Package into container
        File container = null;
        if (ctype.equals("zip")) {
            container = File.createTempFile("nig_pv_upload", ".zip");
            ZipUtil.zip(files, getParentNmrDir(acqDir), container, clevel);
        } else if (ctype.equals("tar")) {
            container = File.createTempFile("nig_pv_upload", ".tar");
            TarUtil.tar(files, getParentNmrDir(acqDir), container, CompressionType.NONE);
        } else if (ctype.equals("aar")) {
            // NB can use for other container types now.
            // However, can add large file support via apache tar not available
            // yet in ArchiveUtil
            container = File.createTempFile("nig_pv_upload", ".aar");
            ArchiveUtil.compress(files, getParentNmrDir(acqDir), container, MimeTypes.AAR, clevel, null);
        }
        //
        return container;
    }
     ...
     * @param filesToZip
     */
    private static void findFidFilesToCompress(File dir, Set<File> filesToZip) {
        // "nmr" is "above" the study..
        // This exits the bottom-up recursive process
        if (dir.getName().toLowerCase().equals("nmr")) {
            return;
        ...
        // Now, go up to the parent..
        boolean fidWithImage = false; // We are only in this function if we are
                                      // uploading the fid to its own DataSet
        findImageFilesToCompress(dir.getParentFile(), filesToZip, fidWithImage);
    }
    ...
    /**
     * Returns the subject (which is in text format and includes bruker
     * metadata) file in the study directory.
     * 
     * @param dir
     * @return
     * @throws Exception
    ...
    /**
     * Returns the free-induction decay (FID) file within the acquisition
     * directory.
     * 
     * @param dir
     * @return
