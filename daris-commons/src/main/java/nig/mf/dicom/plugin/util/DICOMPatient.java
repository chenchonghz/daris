...
public class DICOMPatient {
	...
	/**
	 * Need at least lastName to match
	 * First name is optional (can be null in both)
	 * 
	 * @param findSubectMethod 'id', 'name', 'name+' (name + sex + dob), or 'all' (id, name, dob, sex)
	 ...
	 * @throws Throwable
	 */
	public static boolean matchDICOMDetail (String findSubjectMethod, XmlDoc.Element oldPatientMeta, DicomPersonName newName, 
			String newSex, Date newDOB, String newID) throws Throwable {
		if (oldPatientMeta==null) return false;
		...
		if (findSubjectMethod.equalsIgnoreCase("id")) {
			...
			if (!dicomNamesMatch (newName, oldPatientMeta)) return false;
			if (!stringsMatch (oldPatientMeta.value("sex"), newSex, true)) return false;
			if (!dicomDOBMatch (oldPatientMeta.dateValue("dob"), newDOB)) return false;
			return true;
		} else if (findSubjectMethod.equalsIgnoreCase("all")) {
			if (!stringsMatch (oldPatientMeta.value("id"), newID, true)) return false;
			if (!dicomNamesMatch (newName, oldPatientMeta)) return false;
			if (!stringsMatch (oldPatientMeta.value("sex"), newSex, true)) return false;
			if (!dicomDOBMatch (oldPatientMeta.dateValue("dob"), newDOB)) return false;
			return true;
	...
	public static Boolean dicomDOBMatch (Date oldDOB, Date newDOB) throws Throwable {
	...
	public static boolean stringsMatch (String oldStr, String newStr, boolean ignoreCase) {
	...
	private static Boolean dicomNamesMatch (DicomPersonName newName, XmlDoc.Element oldPatientMeta) throws Throwable {
		// COnstruct fullnames and compare. 
		// If there are CIDs in the name, treat them as null as we don't save/compare CIDs.
		String newFullName = null;
		if (newName!=null) {
			newFullName = makeFullName (newName.first(), newName.last());
			if (CiteableIdUtil.isCiteableId(newFullName)) newFullName = null;
		}
		...
		if (oldPatientMeta!=null) {
			...
			if (CiteableIdUtil.isCiteableId(oldFullName)) oldFullName = null;
