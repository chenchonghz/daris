...
public class DistributedAsset {
	...
	public DistributedAsset(String route, String cid) throws Throwable {
		...
		_route = route;
		try {
			// TBD : remove try/catch when 4.1.001 server installed (has fix for this call failing)
			// THe thread is not used much, general functionality currently not compromised
	...
	public DistributedAsset getParentProject(Boolean readOnly) throws Throwable {
		...
		String pid = nig.mf.pssd.CiteableIdUtil.getProjectId(_cid);
	...
	public String getParentProjectCID() {
		...
		return nig.mf.pssd.CiteableIdUtil.getProjectId(_cid);
	}
	...
	public DistributedAsset getParentSubject(Boolean readOnly) throws Throwable {
		...
		String sid = nig.mf.pssd.CiteableIdUtil.getSubjectId(_cid);
	...
	public String getParentSubjectCID() {
		...
		return nig.mf.pssd.CiteableIdUtil.getSubjectId(_cid);
	}
	...
	/*
	 * Return the cid and proute an an element with value cid and attribute proute
	 * 
	 */
	public XmlDoc.Element asXmlDoc (String name) {
		XmlDoc.Element t = new XmlDoc.Element(name, _cid);
		if (_route!=null) {
			XmlDoc.Attribute a = new XmlDoc.Attribute("proute", _route);
			t.add(a);
		}
		return t;
	}
