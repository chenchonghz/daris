...
public class DistributedAssetUtil {
	...
	public static boolean hasRemoteChildren(ServiceExecutor executor, String proute, String cid,
		...
		if (ptag != null)
			dm.add("ptag", ptag);
		StringBuilder where = new StringBuilder("cid starts with '" + cid + "'");
		DistributedQuery.appendResultAssetTypePredicate(where, assetType);
		dm.add("where", where.toString());
	...
	public static DistributedAsset findReplicaObject(ServiceExecutor executor, String cid, String preferredRoute)
		...
		if (r.element("asset") != null) {
			...
			if (r.element("asset") == null)
				return null;
			//
			List<XmlDoc.Element> els = r.elements("cid");
	...
	/**
	 * Construct the ServerRoute for a particualr peer
	 * 
	 * @param executor
	 * @param peer
	 * @return
	...
	public static ServerRoute findPeerRoute (ServiceExecutor executor, String peer) throws Throwable {
		...
		if (r!=null) {
			...
			if (t==null) {
				throw new Exception("Could not determine route to peer");	
			}
