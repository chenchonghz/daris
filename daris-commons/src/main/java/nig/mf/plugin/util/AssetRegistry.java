 ...
 * should be limited to a single element. 
 * 
 * @author nebk
 *
 */
public class AssetRegistry {
	...
	private static final String ASSETNAMESPACE = "/pssd/registries";
	public enum AccessType {PUBLIC, PRIVATE, ALL};

	/**
	 * Parse the access type string into the enum
	 * 
	 * @param accessType must have value 'public' or 'private'
	 * @return
	 */
	public static AccessType parseAccessType (String accessType) throws Throwable {
	 ...
	 * 
	 * @param executor
	 * @param registryAssetType For public objects, this string is used for both the  asset type and asset name.  It's up to the caller
	 * to define and manage the name.  Must be unique.  For example, "pssd-role-member-registry", 'pssd-dicom-server-registry"
	 * For private objects, it's just the asset type (they don't get a name as there are more than one)
	 * @param accessType specifies whether the registry is the 'public' or 'private' singleton
	 * @return - Asset ID of registry object.  Will be null if does not exist
	 * @throws Throwable
	 */
	public static String findRegistry (ServiceExecutor executor, String registryAssetType, AccessType accessType) throws Throwable {
		...
		if (accessType==AccessType.PUBLIC){ 
			...
			// The asset will have an ACL on it so only the user can find their assets
			String query = "(type='" + registryAssetType + "') and (created by me) and (asset hasno name)";     
			dm.add("where", query);
			dm.add("pdist", 0);               // Force to local query
		}
		...
		if (ids==null || ids.size() == 0) {
			...
			// Trouble
			if (accessType==AccessType.PUBLIC) {
				throw new Exception ("There are multiple public registry objects of name " + registryAssetType + ". Contact the administrator");
			} else {
				throw new Exception ("There are multiple private registry objects of type " + registryAssetType + " for the calling user. Contact the administrator");
			}
		} else {
			id = r.value("id");
		}
	 ...
	 * @throws Throwable
	 */
	public static String findAndCreateRegistry (ServiceExecutor executor, String registryAssetType, AccessType accessType) throws Throwable {

		// We can only return one id at a time...
		if (accessType==AccessType.ALL) {
			throw new Exception ("AccessType 'all' is not supported in function AssetRegistry.findAndCreateRegistry");
		}

		// CHeck user holds the appropriate role to create a public registry object
		if (accessType==AccessType.PUBLIC) checkHasAdminRole(executor);
		...
		if (id==null) {
			...
			dm.add("namespace", new String[] {"create", "true"}, ASSETNAMESPACE);

			if (accessType==AccessType.PRIVATE) {

				dm.add("type", registryAssetType);

				// Add an ACL so only this user can see it and so there will only be one per user
	 ...
	 * @throws Throwable
	 */
	public static String  destroyRegistry (ServiceExecutor executor,  String registryAssetName, AccessType accessType) throws Throwable {
	 ...
	 * @throws Throwable
	 */
	public static void addItem (ServiceExecutor executor, String id, XmlDoc.Element item, String documentType)  throws Throwable {
		XmlDocMaker dm = new XmlDocMaker("args");
		dm.add("id", id);
	 ...
	 * 
	 * @param id the Registry asset id
	 * @param docId the id of the document we want to remove
	 * @param doc - The element to remove
	 * @param documentType The Document Type  of the specific registry elements you are working with (e.g. daris:pssd-dicom-server-registry
	 * @throws Throwable
	 */
	public static void removeItem (ServiceExecutor executor, String id, String docId,  XmlDoc.Element item, String documentType)  throws Throwable {
	 ...
	 * @param the Xpath to iterate over. E.g. asset/meta/daris:pssd-role-member-registry/role
	 * @param w
	 * @throws Throwable
	 */
	public static void list (ServiceExecutor executor, String id, String xpath, XmlWriter w) throws Throwable {
	 ...
	 * @param accessType specifies whether the registry is the 'public' or 'private' singleton
	 * @param checkTopName  If true, compares the names of the elements at the top level, if false, does not. 
	 * @return  true or false
	 * @throws Throwable
	 */
	public static boolean checkExists (ServiceExecutor executor,  String registryAssetName, String documentType, XmlDoc.Element item, AccessType accessType, Boolean checkTopName) throws Throwable {
	...
	/**
	 * Find out if the specified item is a member of the registry
	 * It critically depends on the elements of the item being in the same
	 * order as in the registry. Services like DvcDICCOMAEAdd make sure of this.
	 * 
	 * @param executor
	 * @param id Registry asset id
	 * @param documentType The Document Type  of the specific registry elements you are working with (e.g. pssd-dicom-server-registry)
	 * @param item The item to test
	 * @param checkTopName  If true, compares the names of the elements at the top level, if false, does not. 
	 ...
	 * @throws Throwable
	 */
	public static String checkExists (ServiceExecutor executor,  String id, String documentType, XmlDoc.Element item, Boolean checkTopName) throws Throwable {
	...
	private static void addACL (XmlDocMaker dm, String role, Boolean readWrite) throws Throwable {
		dm.push("acl");
	...
	private static void addUserAcl (ServiceExecutor executor, XmlDocMaker dm) throws Throwable {
	...
	private static void checkHasAdminRole (ServiceExecutor executor) throws Throwable {
		...
		if (!PSSDUtil.hasRole(route, executor, Role.objectAdminRoleName()) && 
				!PSSDUtil.hasRole(route, executor, Role.powerModelUserRoleName())) {
			throw new Exception ("Caller must have the 'daris:pssd.object.admin' or 'daris:pssd.model.power.user' role to create public registry objects");
		}
	}


	/**
	 * This function finds out if an item already exists in the registry.
	 * It critically depends on the elements of the item being in the same
	 * order as in the registry. Services like DvcDICCOMAEAdd make sure of this.
	 * 
	 * @param executor
	 ...
	 * @throws Throwable
	 */
	private static String hasItem (ServiceExecutor executor, Collection<XmlDoc.Element> docs,  XmlDoc.Element item, Boolean checkTopName) throws Throwable {
		...
		for (XmlDoc.Element doc : docs) {
			...
			if (elements!=null) {
				for (XmlDoc.Element el : elements)	 {
					...
					// If we don't want to compare names, just make them the same :-)
					if (!checkTopName) {
						el.setName(item.name());
					}
					
					// Note that the elements must be in the same order for this String comparison to work.
					// This is why SvcDICOMAE{Add,Remove} re-order the element to the same order
...
/*					
					...
					System.out.println("   Result string compare   = " + s1.equals(s2));
*/
					if (s1.equals(s2)) {
						return doc.value("@id");   
					}
	...
	private static XmlDoc.Element getRegistryAsset (ServiceExecutor executor, String id) throws Throwable {
		XmlDocMaker dm = new XmlDocMaker("args");
		dm.add("id", id);
