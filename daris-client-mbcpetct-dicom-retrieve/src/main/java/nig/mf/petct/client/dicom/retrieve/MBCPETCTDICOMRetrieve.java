...
public class MBCPETCTDICOMRetrieve {
	...
	public static void main(String[] args) throws Throwable {
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equalsIgnoreCase(HELP_ARG)) {
				...
				System.err.println("MBCPETCTDICOMRetrieve: error: unexpected argument = " + args[i]);
		...
		if (ops.modality!=null) {
			...
			if (!ops.modality.equals("PT") && !ops.modality.equals("CT") && !ops.modality.equals("NM")) {
				throw new Exception ("*** Modality must be one of 'PT','CT', 'NM'");
			}
		}
		// Can select by attributes or by asset ID
		if (ops.firstName!=null || ops.lastName!=null || ops.startDate!=null || ops.endDate!=null ||
				ops.modality!=null || ops.description!=null || ops.protocol!=null) ops.hasAttributes = true;
		if (ops.hasAttributes) {
			if (ops.patientAsset!=null || ops.studyAsset!=null || ops.seriesAsset!=null) {
				throw new Exception ("*** You cannot give selection attributes as well as asset IDs");
			}
		} else {
			if (ops.patientAsset==null && ops.studyAsset==null && ops.seriesAsset==null) {
				throw new Exception ("*** You must supply either selection attributes or asset IDs");
			}
		}


		// DICOM AE defaults to console
		if (ops.calledAET==null) ops.calledAET = "MBIC-PET-CONSOLE";   // The AET of the PET COnsole
		if (ops.host==null) ops.host = "172.30.31.75";
		if (ops.port==null) ops.port = "104";
		//
		if (ops.callingAET==null) ops.callingAET = "UM-DaRIS-1";       // THe AET of the DaRIS1 system 
		...
		try {
			...
			System.err.println("MBCPETCTDICOMRetrieve: error: " + t.getMessage());
			t.printStackTrace();
			System.exit(1);
		}
	}
	...
	private static void push (Connection cxn, Options ops) throws Throwable {
		...
		w.push("ae");  // Destination
		w.add("aet", ops.calledAET);
		w.add("host", ops.host);
		w.add("port", ops.port);
		w.pop();
		//
		w.add("aet", ops.callingAET);                     // Sender
		w.add("exception-on-fail", true);
		if (ops.patientAsset!=null) {
			w.add("patient", ops.patientAsset);
			if (ops.list) System.out.println("Patient asset = " + ops.patientAsset);
		} else if (ops.studyAsset!=null) {
			w.add("study", ops.studyAsset);
			if (ops.list) System.out.println("Study asset = " + ops.studyAsset);
		} else if (ops.seriesAsset!=null) {
			w.add("series", ops.seriesAsset);
			if (ops.list) System.out.println("Series asset = " + ops.seriesAsset);
		}
		//
		if (!ops.list) {
			System.out.println("Sending data");
			cxn.execute("dicom.send", w.document());
		}
	}

	private static void findAndPush (Connection cxn, Options ops) throws Throwable {
		Vector<String> seriesAssets = new Vector<String>();
		//
		String query = nameSpaceQuery();

		// If there is a patient name query do that first and then find the Series
		XmlStringWriter w = new XmlStringWriter ();
				...
				if (seriesIDs!=null && seriesIDs.size()>0) {
					...
					if (ops.list) {
						for (String seriesAsset : seriesAssets) {
							System.out.println("Series asset id = " + seriesAsset);
						}
					}
				}
			}
	...
	private static void addSeries (Connection cxn, Options ops, String patientAsset, Collection<String> seriesAssets) throws Throwable {

		// Find the Studies for this patient
		XmlStringWriter w = new XmlStringWriter();
		String query = nameSpaceQuery() + 
				" and (mf-dicom-study has value and (related to{had-by} (id=" + patientAsset + ")))";
		w.add("where", query);
		w.add("size", "infinity");
		XmlDoc.Element r = cxn.execute("asset.query", w.document());
		if (r==null) return;
		...
		if (studyAssets.size()>0) {
			for (String studyAsset : studyAssets) {
				...
				query = nameSpaceQuery() + 
						" and (mf-dicom-series has value and (related to{container} (id=" + studyAsset + ")))";
					...
					if (seriesIDs!=null && seriesIDs.size()>0) {
						...
						if (ops.list) {
							for (String seriesID : seriesIDs) {
								System.out.println("      Series asset id = " + seriesID);
							}
						}
					}
				}
	...
	private static String addExtraQueries (String query, Options ops) {	
		...
		if (ops.description!=null) {
			query += " and xpath(mf-dicom-series/description) contains literal ('" + ops.description + "')";
		}
		if (ops.protocol!=null) {
			query += " and xpath(mf-dicom-series/protocol) contains literal ('" + ops.protocol + "')";
		}
		return query;
	}
	...
	private static void printHelp(PrintStream os) {
		...
		os.println();
		os.println("Synopsis:");
		os.println("   Finds and pushes DICOM data from Mediaflux to the give AE.  ");
		os.println();
		os.println("Usage:");
		os.println("   " + MBCPETCTDICOMRetrieve.class.getName() + " [options..]");
		os.println();
		os.println();
		...
		os.println("    -mf.password  [Required]: The logon user's obfuscated password.");
		...
		os.println("   " + PROTOCOL_ARG + "      Specify protocol of series (free text; optional; partial matches allowed)");
		os.println("   " + PATIENT_ASSET_ARG + "       Specify Mediaflux Patient asset ID (all children of patient will be pushed)");
		os.println("   " + STUDY_ASSET_ARG + "         Specify Mediaflux Study asset ID (all children of patient will be pushed)");
		os.println("   " + SERIES_ASSET_ARG + "        Specify Mediaflux Series asset ID");
		os.println("   " + CALLING_AET_ARG + "    AE Title of client. Defaults to 'UM-DaRIS-1'");
		os.println("   " + CALLED_AET_ARG + "     AE Title of destination DICOM server. Defaults to 'MBIC-PET-CONSOLE'");
		os.println("   " + HOST_ARG + "          Host IP of destination DICOM server. Defaults to '172.30.31.75'");
		os.println("   " + PORT_ARG + "          Port  of destination DICOM server. Defaults to '104'");
