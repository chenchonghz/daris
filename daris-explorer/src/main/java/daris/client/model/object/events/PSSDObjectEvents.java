...
public class PSSDObjectEvents {
    ...
    public static void initialize() {
        ...
        SystemEventRegistry.add(PSSDObjectEvent.SYSTEM_EVENT_NAME,
                new SystemEventFactory() {
                    public SystemEvent instantiate(String type,
                            XmlDoc.Element ee) throws Throwable {
                        String object = ee.value("object");
                        PSSDObjectEvent.Action action = PSSDObjectEvent.Action
                                .valueOf(ee.value("action").toUpperCase());
                        int nbc = ee.intValue("object/@nbc", -1);
                        return new PSSDObjectEvent(new DObjectRef(object, nbc),
                                action);
                    }
                });
